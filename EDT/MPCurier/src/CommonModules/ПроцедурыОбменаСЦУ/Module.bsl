...
//		Возврат ОтветМетода;
	...
	Если ДанныеВызова.Свойство("arrayOfOrganizations") Тогда
		...
		Если МассивКодов.Количество() <> 0 Тогда
			Возврат СнятьРегистрациюСОбъектовЦУ(ОбщееСоединениеHTTP, МассивКодов, IDУзла, "Справочник.Организации");
		Иначе
			Возврат Истина;
		КонецЕсли;				
	КонецеСли;
...
Функция ВыполнитьОбмен_ПолучитьСписокВодителей(ОбщееСоединениеHTTP = "", IDУзла) Экспорт
	...
	Если ДанныеВызова.Свойство("arrayOfDrivers") Тогда
		...
		Если МассивКодов.Количество() <> 0 Тогда
			Возврат СнятьРегистрациюСОбъектовЦУ(ОбщееСоединениеHTTP, МассивКодов, IDУзла, "Справочник.новаВодители");
		Иначе
			Возврат Истина;
		КонецЕсли;				
	КонецеСли;
...
Функция ВыполнитьОбмен_ПолучитьСписокЭкспедиторов(ОбщееСоединениеHTTP = "", IDУзла) Экспорт
	...
	Если ДанныеВызова.Свойство("arrayOfForwarders") Тогда
		...
		Если МассивКодов.Количество() <> 0 Тогда
			Возврат СнятьРегистрациюСОбъектовЦУ(ОбщееСоединениеHTTP, МассивКодов, IDУзла, "Справочник.новаЭкспедиторы");
		Иначе
			Возврат Истина;
		КонецЕсли;				
	КонецеСли;
...
Функция ВыполнитьОбмен_ПолучитьСписокТранспорта(ОбщееСоединениеHTTP = "", IDУзла) Экспорт
	...
	Если ДанныеВызова.Свойство("arrayOfCars") Тогда
		Для Каждого ТекТранспорт Из ДанныеВызова.arrayOfCars Цикл
			...
			ТранспортОбъект.Регион = Справочники.РегиональныеТерминалы.НайтиПоКоду(ТекТранспорт.id);
			ТранспортОбъект.ПометкаУдаления = ТекТранспорт.deletionMark;
			ТранспортОбъект.Записать();
			МассивКодов.Добавить(ТекТранспорт.id);
		КонецЦикла;
		
		//TODO: здесь был код по установке пароля по данным авто Константа.Пароль
		
		Если МассивКодов.Количество() <> 0 Тогда
			Возврат СнятьРегистрациюСОбъектовЦУ(ОбщееСоединениеHTTP, МассивКодов, IDУзла, "Справочник.новаТранспорт");
		Иначе
			Возврат Истина;
		КонецЕсли;				
	КонецеСли;
...
Функция ВыполнитьОбмен_ПолучитьСписокРезультатовДоставки(ОбщееСоединениеHTTP = "", IDУзла) Экспорт
	...
	Если ДанныеВызова.Свойство("arrayOfDeliveryResults") Тогда
		...
		Если МассивКодов.Количество() <> 0 Тогда
			Возврат СнятьРегистрациюСОбъектовЦУ(ОбщееСоединениеHTTP, МассивКодов, IDУзла, "Справочник.ПричиныНеВыполненияДоставки");
		Иначе
			Возврат Истина;
		КонецЕсли;				
...
Функция ВыполнитьОбмен_ПолучитьСписокТарифовИОбъемныхБрейков(ОбщееСоединениеHTTP = "", IDУзла) Экспорт
	...
	Если ДанныеВызова.Свойство("arrayOfTariffs") Тогда
		...
		КонецЦикла;
		Если МассивКодов.Количество() <> 0 Тогда
			Возврат СнятьРегистрациюСОбъектовЦУ(ОбщееСоединениеHTTP, МассивКодов, IDУзла, "Справочник.ТарифныеСетки");
		Иначе
			Возврат Истина;
		КонецЕсли;					
	КонецЕсли;
...
Функция ВыполнитьОбмен_КалендариДоставок(ОбщееСоединениеHTTP = "", IDУзла) Экспорт
	...
	Если ДанныеВызова.Свойство("arrayOfCalendars") Тогда	
		...
		Если МассивКодов.Количество() <> 0 Тогда
			Возврат СнятьРегистрациюСОбъектовЦУ(ОбщееСоединениеHTTP, МассивКодов, IDУзла, "Справочник.КалендариДоставки");
		Иначе
			Возврат Истина;
		КонецЕсли;
	Конецесли;
