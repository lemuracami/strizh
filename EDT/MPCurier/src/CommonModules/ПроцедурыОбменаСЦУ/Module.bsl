...
Функция ВыполнитьОбмен_ПолучитьЗаказыПоРейсу(ОбщееСоединениеHTTP = "", IDTrip) Экспорт
	...
	Попытка
		...
		ВозниклаОшибка = Ложь;
		ДанныеВызова = ДанныеJSONВСтруктуру(РезультатВызова, "date,dateIdRowOrder", ВозниклаОшибка);
		Если ВозниклаОшибка Тогда
			ОтветМетода.Вставить("РезультатВызова", Ложь);
...
//		СтруктураЗаказШаблон = Новый ФиксированнаяСтруктура("number, date, costDeliveryRequiredIfRefusal, calendarID, dateIdRowOrder, deliveryCostCalculations, internetShopID, status, timeOfDelivery, lat, long, phone, address
...
//		СтруктураСтрокиТоваровШаблон = Новый ФиксированнаяСтруктура("artikul, code, name, count, summa, price, barcode, markingCode, idGoodSeller, NDSRow");
		...
		ТаблицаЗаказов.Колонки.Добавить("Заказ", Новый ОписаниеТипов(Тип("СправочникСсылка.Заказы")));
		ТаблицаЗаказов.Колонки.Добавить("НомерЗаказа", Новый ОписаниеТипов(Тип("Число"),,, Новый КвалификаторыЧисла(20)));
		ТаблицаЗаказов.Колонки.Добавить("КодКалендарьДоставки", Новый ОписаниеТипов(Тип("Строка"),,,, Новый КвалификаторыСтроки(9)));
		ТаблицаЗаказов.Колонки.Добавить("ТребуетсяОплатаСтоимостиДоставкиПриОтменеЗаказа", Новый ОписаниеТипов(Тип("Булево")));
		ТаблицаЗаказов.Колонки.Добавить("ДатаВремяПоследнейМаршрутизацииВМП", Новый ОписаниеТипов(Тип("Дата"),,,,,Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
		
		
		Если ДанныеВызова.Свойство("arrayOfOrders") Тогда	
			Для Каждого ТекЗаказ Из ДанныеВызова.arrayOfOrders Цикл
				СписокНомеровЗаказов.Добавить(ТекЗаказ.number);
			КонецЦикла;
		Иначе
			ОтветМетода.Вставить("РезультатВызова", Ложь);
		...
		|	Заказы.Код КАК НомерЗаказа,
		|	Заказы.Ссылка КАК Заказ
		|ИЗ
		|	Справочник.Заказы КАК Заказы
		|ГДЕ
		|	Заказы.Код В (&СписокКодов)");
		Зап.УстановитьПараметр("СписокКодов", СписокНомеровЗаказов); 
		ТабЗаказов = Зап.Выполнить().Выгрузить();
		
		Для Каждого ТекЗаказ Из ДанныеВызова.arrayOfOrders Цикл
			ПараметрыПоиска = Новый Структура("НомерЗаказа", ТекЗаказ.number);
			НайденныеСтроки = ТабЗаказов.НайтиСтроки(ПараметрыПоиска);
			Если НайденныеСтроки.Количество() = 0 Тогда
				НовыйЗаказ = Справочники.Заказы.СоздатьЭлемент();
				НовыйЗаказ.Код = Число(ТекЗаказ.number);
			Иначе
				НовыйЗаказ = НайденныеСтроки[0].Заказ.ПолучитьОбъект();	
			КонецеСли;
			
			НовыйЗаказ.ДатаЗаказа = 												ТекЗаказ.date;
			НовыйЗаказ.ТребуетсяОплатаСтоимостиДоставкиПриОтменеЗаказа = 			ТекЗаказ.costDeliveryRequiredIfRefusal;
			НовыйЗаказ.КалендарьДоставки = 											ТекЗаказ.calendarID;
			НовыйЗаказ.ДатаЗаказа = 												ТекЗаказ.date;
			НовыйЗаказ.ДатаЗаказа = 												ТекЗаказ.date;
			НовыйЗаказ.ДатаЗаказа = 												ТекЗаказ.date;
			НовыйЗаказ.ДатаЗаказа = 												ТекЗаказ.date;
