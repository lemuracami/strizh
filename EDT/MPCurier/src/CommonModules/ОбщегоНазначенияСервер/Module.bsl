...
// Справочник регионов пустой.
// 
// Возвращаемое значение:
//  Булево - Справочник регионов пустой
...
// Получить список значений региональных терминалов.
// 
// Возвращаемое значение:
//  СписокЗначений - Получить список значений региональных терминалов
Функция ПолучитьСписокЗначенийРегиональныхТерминалов() Экспорт
	...
	Пока Выб.Следующий() Цикл
		СписокРегионов.Добавить(Выб.Регион);
	КонецЦикла;		
	Возврат СписокРегионов;
КонецФункции

// Установлено значение региона работы МП.
// 
// Возвращаемое значение:
//  Булево - Установлено значение региона работы МП
Функция УстановленоЗначениеРегионаРаботыМП() Экспорт
	Возврат ЗначениеЗаполнено(Константы.РегионМП.Получить());
КонецФункции	

// Транспорт входа ВМПринадлежит региону работы МП.
// 
// Возвращаемое значение:
//  Булево - Транспорт входа ВМ принадлежит региону работы МП
Функция ТранспортВходаВМППринадлежитРегионуРаботыМП() Экспорт
	Зап = Новый Запрос("ВЫБРАТЬ
	|	РегионМП.Значение КАК Значение
	|ИЗ
	|	Константа.РегионМП КАК РегионМП
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константа.ТранспортМП КАК ТранспортМП
	|		ПО ТранспортМП.Значение.Регион = РегионМП.Значение");
	Возврат Не Зап.Выполнить().Пустой() 
		Или Не ЗначениеЗаполнено(Константы.ТранспортМП.Получить()) 
		Или Не ЗначениеЗаполнено(Константы.РегионМП.Получить()); 
КонецФункции

// Очистить транспорт МП.
Процедура ОчиститьТранспортМПИДанныеАвтологина() Экспорт
	Константы.ТранспортМП.Установить(Справочники.Транспорт.ПустаяСсылка());
	ПараметрыСеанса.ТранспортМП = Справочники.Транспорт.ПустаяСсылка();
	
	Константы.СохраненныйПарольВходаВМП.Установить("");
КонецПроцедуры	

// Получить описание версии из макета версий.
// 
// Параметры:
//  НомерВерсии - Строка - Номер версии
// 
// Возвращаемое значение:
//  Структура - Получить описание версии из макета версий:
// * Описание - ФорматированнаяСтрока, Строка - 
// * ЭтоРелизнаяВерсия - Булево - 
...
// Получить шрифт предупреждение пользователю.
// 
// Возвращаемое значение:
//  Шрифт - Получить шрифт предупреждение пользователю
Функция ПолучитьШрифтПредупреждениеПользователю() Экспорт
	Возврат ШрифтыСтиля.ПредупреждениеПользователюШрифт.ПолучитьАбсолютный();
КонецФункции		
#КонецОбласти	
