...
Функция ПроверитьИУстановитьИзначальныеПараметрыМА() Экспорт
	//ВыполнитьОбменПриПервомЗапускеМП();
...
// Выполнить обмен при запуске МП.
//  
Процедура ВыполнитьОбменПриПервомЗапускеМП() Экспорт
	ОбменДаннымиЦУВызовСервера.ВыполнитьОбмен();
КонецПроцедуры	
...
Функция ПроизвестиПервоначальноеЗаполнениеДанных()
	Попытка
		...
		УстановитьЗначенияРеквизитовПредопределенныхЭлементовСправочников();
		ЗаполнитьДанныеСистемыГеопозиционирования();
...
// Установить значения реквизитов предопределенных элементов справочников.
Процедура УстановитьЗначенияРеквизитовПредопределенныхЭлементовСправочников()
	
	ОбъектДоставлен = Справочники.РезультатыДоставки.Доставлен.ПолучитьОбъект();
	ОбъектДоставлен.ЭтоДоставка = Истина;
	ОбъектДоставлен.Забор = Истина;
	ОбъектДоставлен.Записать();	
	
КонецПроцедуры	

// Заполнить данные системы геопозиционирования.
Процедура ЗаполнитьДанныеСистемыГеопозиционирования()
	Если Не ЗначениеЗаполнено(Константы.ВыборПровайдераГеопозиционирования.Получить()) Тогда		
		Константы.ВыборПровайдераГеопозиционирования.Установить(Перечисления.ИспользуемыйПровайдерГеопозиционирования.СамыйТочныйПровайдер);
	КонецЕсли;	
КонецПроцедуры	
