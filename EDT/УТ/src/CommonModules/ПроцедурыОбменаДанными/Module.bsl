...
Функция СформироватьЧастьВызоваОбновленияКонфигурации(Знач ИмяАдминистратораИБ = "", Знач ИмяФайлаИнформацииОбОшибках = "", 
	...
	ЭтаБазаФайловая = ОпределитьЭтаИнформационнаяБазаФайловая();
	Если ЭтаБазаФайловая Тогда
		ТекстФайлаПоУмолчанию = ТекстФайлаПоУмолчанию + "
			|rem Завершение работы пользователей (отключено в клиент-файловом режиме)
			|rem " + ТекстЗавершенияРаботыПользователей;
	Иначе
		ТекстФайлаПоУмолчанию = ТекстФайлаПоУмолчанию + "
			|rem Завершение работы пользователей
			|" + ТекстЗавершенияРаботыПользователей;
	КонецЕсли;	
	ТекстФайлаПоУмолчанию = ТекстФайлаПоУмолчанию +	"
        |
		|start """" /wait %v8exe% CONFIG     %DataBase% %AdminUser% /UpdateDBCfg /UCПакетноеОбновлениеКонфигурацииИБ	" + СтрокаДляВыводаДанныхВФайл + "
        |";
	ТекстРазрешенияРаботыПользователей = "start """" /wait %v8exe% ENTERPRISE %DataBase% %AdminUser% /CРазрешитьРаботуПользователей	/UCПакетноеОбновлениеКонфигурацииИБ	" + СтрокаДляВыводаДанныхВФайл + " /DisableStartupMessages";
	Если ЭтаБазаФайловая Тогда
		ТекстФайлаПоУмолчанию = ТекстФайлаПоУмолчанию + "
			|rem Разрешение работы пользователей (отключено в клиент-файловом режиме)
			|rem " + ТекстРазрешенияРаботыПользователей;
	Иначе
		ТекстФайлаПоУмолчанию = ТекстФайлаПоУмолчанию + "
			|rem Разрешение работы пользователей
			|" + ТекстРазрешенияРаботыПользователей;
	КонецЕсли;	
	ТекстФайлаПоУмолчанию = ТекстФайлаПоУмолчанию +	"
        |
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		Иначе
			...
			Если МассивНайденныхФайлов.Количество() > 0 Тогда
				...
				СтрокаМаксимальныйРазмерПакета  = ?(СтруктураНастроекОбменаДанными.ДанныеНастройки.МаксимальныйРазмерОтправляемогоПолучаемогоПакетаЧерезFTP > 0, "("+СтруктураНастроекОбменаДанными.ДанныеНастройки.МаксимальныйРазмерОтправляемогоПолучаемогоПакетаЧерезFTP+" Кб)","");
				СтрокаДатаПоследнегоФайлаОбмена = ?(ЗначениеЗаполнено(СтруктураНастроекОбменаДанными.ДатаПоследнегоФайлаОбмена), "("+Формат(СтруктураНастроекОбменаДанными.ДатаПоследнегоФайлаОбмена, "ДФ=дд.ММ.гггг") +")", "");
				
				//файлы то есть но ни один не подходит
				СообщениеДляПользователя = "Найденные файлы на FTP не соответствуют параметрам для выполнения обмена данными:
                 | - расширение файла должно быть «XML» или «ZIP»;
                 | - размер файла должен быть больше 0 байт;
                 | - если в настройке обмена указано значение максимального пакета отличное от 0, то размер файла не должен превышать указанного в настройке значения "+ СтрокаМаксимальныйРазмерПакета + ";
                 | - дата создания файла на FTP должна быть больше даты файла, по которому последний раз происходил обмен данными "+СтрокаДатаПоследнегоФайлаОбмена + ".";
