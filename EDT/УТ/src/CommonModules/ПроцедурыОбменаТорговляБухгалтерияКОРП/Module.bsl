...
Функция ВернутьМассивВсехУзловИзПараметровСеанса()
	...
	Возврат ПараметрыСеанса.ВсеУзлыОбменаТорговляБухгалтерияКОРП;
	
КонецФункции

Функция ПолучитьСоответствиеУзловИОрганизаций() Экспорт
	
	Хранилище = ПараметрыСеанса.СоответствиеОрганизацийИУзловОбменаТорговляБухгалтерияКОРП;
...
Функция ПолучитьСоответствиеУзловИСкладов() Экспорт
	...
	Хранилище = ПараметрыСеанса.СоответствиеСкладовИУзловОбменаТорговляБухгалтерияКОРП;
...
Функция ПолучитьСоответствиеУзловИПодразделений() Экспорт
	...
	Хранилище = ПараметрыСеанса.СоответствиеПодразделенийИУзловОбменаТорговляБухгалтерияКОРП;
...
Функция ПолучитьТаблицуУзловИДат() Экспорт
	...
	Хранилище = ПараметрыСеанса.СоответствиеУзловИДатДляОбменаБухгалтерияКОРПТорговля;
...
Функция ПолучитьЗапросомМассивУзловПоОрганизации(Организация) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ Различные
	               ...
	               |ИЗ
	               |	ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП КАК ОбменУправлениеТорговлейБухгалтерияКОРП
				   |		Левое соединение ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП.Организации КАК СписокОрганизаций
				   |			ПО (ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка = СписокОрганизаций.Ссылка)
				   |
	               |ГДЕ
...
Функция ПолучитьЗапросомМассивУзловПоСкладу(Склад) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ Различные
	               ...
	               |ИЗ
	               |	ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП КАК ОбменУправлениеТорговлейБухгалтерияКОРП
				   |		Левое соединение ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП.Склады КАК СписоСкладов
				   |			ПО (ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка = СписоСкладов.Ссылка)
				   |
	               |ГДЕ
...
Функция ПолучитьЗапросомМассивУзловПоПодразделению(Подразделение) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ Различные
	               ...
	               |ИЗ
	               |	ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП КАК ОбменУправлениеТорговлейБухгалтерияКОРП
				   |		Левое соединение ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП.Подразделения КАК СписокПодразделений
				   |			ПО (ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка = СписокПодразделений.Ссылка)
				   |
	               |ГДЕ
...
Процедура ДополнитьМассивУзловДляВыгрузкиДокумента(Элемент, МассивУзловДляПередачи, 
	...
	Если ТипЭлемента = Тип("ДокументСсылка.АвансовыйОтчет")
		...
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.ПоступлениеТоваровУслуг")
		ИЛИ ТипЭлемента = Тип("ДокументСсылка.КорректировкаПоступления")
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.КорректировкаПоступления") Тогда
		
		Если ТипЭлемента = Тип("ДокументСсылка.КорректировкаПоступления")
			ИЛИ ТипЭлемента = Тип("ДокументОбъект.КорректировкаПоступления")
			И ТипЗнч(Элемент.СкладОрдер) = Тип("СправочникСсылка.Склады")
		Тогда
	...
	ИначеЕсли ТипЭлемента = Тип("ДокументСсылка.АккредитивПереданный")
		...
		ИЛИ ТипЭлемента = Тип("ДокументОбъект.ДокументРасчетовСКонтрагентом") Тогда
	...
	Если НЕ (
			...
			ИЛИ ТипЭлемента = Тип("ДокументОбъект.ВводНачальныхОстатковНДСпоПартиям")
			ИЛИ ТипЭлемента = Тип("ДокументСсылка.ДокументРасчетовСКонтрагентом")
			ИЛИ ТипЭлемента = Тип("ДокументОбъект.ДокументРасчетовСКонтрагентом")
	...
	Если ОтражатьВБухгалтерскомУчете = Ложь	Тогда
		...
		МассивУзловБезОграничений = ПараметрыСеанса.УзлыДляОбменаТорговляБухгалтерияБезОтбораПоОтражениюВБухУчетеКОРП;
...
Процедура ПередЗаписьюСсылочногоТипаДляОбменаПоОрганизации(Источник, Отказ, ИмяБазовогоТипа, 
	...
	Если Отказ
		ИЛИ НЕ ПараметрыСеанса.НаличиеОбменаТорговляБухгалтерияКОРП Тогда
...
Процедура ПередУдалениемДокументаДляОбменаТорговляБухгалтерияКОРППередЗаписью(Источник, Отказ) Экспорт
...
Процедура ПередЗаписьюДокументаДляОбменаТорговляБухгалтерияКОРППередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
...
Процедура ПередЗаписьюРегистраСведенийОбменТорговляБухгалтерияКОРППередЗаписью(Источник, Отказ, Замещение) Экспорт
...
Процедура ПередЗаписьСправочникаДляОбменаТорговляБухгалтерияКОРППередЗаписью(Источник, Отказ) Экспорт
...
Процедура ПередУдалениемСправочникаДляОбменаТорговляБухгалтерияКОРППередУдалением(Источник, Отказ) Экспорт
...
Процедура ПриЗаписиРегистрыНакопленияДляОбменаТорговляБухгалтерияКОРППриЗаписи(Источник, Отказ, Замещение) Экспорт
	...
	Если ТипРегистратора = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
		...
		ФиксированныйМассивУзловДлярегистрации = ПараметрыСеанса.УзлыДляОбменаТорговляБухгалтерияРегистрацияДокументовПриДвиженииПартийКОРП;
