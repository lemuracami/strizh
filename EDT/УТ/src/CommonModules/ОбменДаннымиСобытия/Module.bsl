...
//
Процедура ПриОтправкеДанных(
				ЭлементДанных,
				ОтправкаЭлемента,
				УзелИнформационнойБазы,
				Знач ИмяПланаОбмена,
				Знач СозданиеНачальногоОбраза
	)
	...
	Если    ИмяБазовогоТипа = ОбщегоНазначения.ИмяТипаСправочники()
		...
		Если МассивУзловДляРегистрацииОбъекта.Найти(УзелИнформационнойБазы) = Неопределено Тогда
...
Процедура ВыполнитьПравилоРегистрацииОбъектаДляНабораЗаписей(МассивУзловРезультат,
	...
	Если Замещение И Не Выгрузка Тогда
		...
		СтарыйНаборЗаписей = НаборЗаписейПоТипу(ОбъектМетаданных);
...
Процедура УстановитьЗначенияПоУмолчаниюНаУзле(УзелПланаОбмена, Настройки) Экспорт
	...
	НастройкаОтборов = ОбменДаннымиСервер.ЗначениеПоТипу(ПланыОбмена[ИмяПланаОбмена].ЗначенияПоУмолчаниюНаУзле(), "Структура");
	
	УстановитьЗначенияНаУзле(УзелПланаОбмена, Настройки, НастройкаОтборов);
	
КонецПроцедуры

Процедура УстановитьЗначенияНаУзле(УзелПланаОбмена, Настройки, НастройкаОтборов)
...
Процедура УстановитьЗначенияОтборовНаУзле(УзелПланаОбмена, Настройки) Экспорт
	...
	НастройкаОтборов = ОбменДаннымиСервер.ЗначениеПоТипу(ПланыОбмена[ИмяПланаОбмена].НастройкаОтборовНаУзле(), "Структура");
...
//                                    для методов ПриОтправкеДанныхГлавному() и ПриОтправкеДанныхПодчиненному()
//
Процедура ПриОтправкеДанныхКорреспонденту(ЭлементДанных, ОтправкаЭлемента, Знач СозданиеНачальногоОбраза = Ложь, Получатель = Неопределено) Экспорт
	...
	ИначеЕсли ОбменДаннымиПовтИсп.ЭтоУзелОбменаДаннымиБСП(Получатель.Ссылка) Тогда
		...
		ПриОтправкеДанных(
						...
						ОтправкаЭлемента,
						Получатель.Ссылка,
						ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(Получатель.Ссылка),
...
Функция РазделенныеДанные(ОбъектМетаданных)
	...
	Возврат ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(ОбъектМетаданных.ПолноеИмя());
	
КонецФункции

Функция РазделенныйПланОбмена(Знач ИмяПланаОбмена)
	
	Возврат ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных("ПланОбмена." + ИмяПланаОбмена);
