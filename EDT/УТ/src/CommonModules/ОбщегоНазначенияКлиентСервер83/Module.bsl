...
//		Результат.ПодробноеОписаниеОшибки = СтрШаблон(СтрокаСообщенияОбОшибке, ПодробноеПредставлениеОшибки(Информация));
//		Результат.КраткоеОписаниеОшибки = СтрШаблон(СтрокаСообщенияОбОшибке, КраткоеПредставлениеОшибки(Информация));
//		
//		Возврат Результат;
//	КонецПопытки;
//	
...
//	
//	МассивПодстрок = СтрРазделить(СтрокаПереданныхАдресов, ";", Ложь);
...
//			ДопМассивПодстрок = СтрРазделить(СтрокаПереданныхАдресов, ",");
...
//	КонецЦикла;
//	
//	Если МассивПодстрокКОбработке.Количество() > 1 
//		И ПустаяСтрока(СокрЛП(МассивПодстрокКОбработке[МассивПодстрокКОбработке.Количество() - 1])) Тогда
//		
//		МассивПодстрокКОбработке.Удалить(МассивПодстрокКОбработке.Количество() - 1);
//		
//	КонецЕсли;
//	
...
//							ВызватьИсключение СтрШаблон(СообщениеНедопустимыеСимволы,
...
//					ВызватьИсключение СтрШаблон(СообщениеНеверныйФорматПочтовогоАдреса,СтрокаАдреса);
...
//						ВызватьИсключение СтрШаблон(СообщениеНедопустимыеСимволы,Символ,СтрокаАдреса);
//					КонецЕсли;
//				КонецЕсли;
//				
//				Накопитель = Накопитель + Символ;
...
//			ВызватьИсключение СтрШаблон(СообщениеНеверныйФорматПочтовогоАдреса, ПолноеИмяАдресата);
//		ИначеЕсли СтрЧислоВхождений(ПочтовыйАдрес, "@") <> 1 И ВызыватьИсключение Тогда 
//			ВызватьИсключение СтрШаблон(СообщениеНеверныйФорматПочтовогоАдреса,ПочтовыйАдрес);
//		КонецЕсли;
//		
