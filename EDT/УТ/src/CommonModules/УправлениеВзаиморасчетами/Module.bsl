...
Процедура ОтражениеОплатыВРегистреОперативныхРасчетовПоДокументам(СтруктураШапкиДокумента, ДатаДвижений, ИмяТаблицыПлатежейИлиТаблица, ВидРасчетовСКонтрагентом, ВидДвижения, Движения, Отказ, Заголовок) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов.Остатки(, &ТекстФильтра) КАК ВзаиморасчетыСКонтрагентамиПоДокументамОстатки
	|ГДЕ
	...
	Для каждого СтрокаПлатежа Из ТаблицаПлатежей Цикл
		...
		Если ЭтоПогашениеКредДокумента Тогда
			ТекстФильтра = ТекстФильтра + " ИЛИ (ДоговорКонтрагента = &ДоговорКонтрагента" + ТекНомер 
			                            + " И Сделка = &Сделка" + ТекНомер
			                            + " И ДокументРасчетовСКонтрагентом = &ДокументРасчетовСКонтрагентом" + ТекНомер
			                            + " И ВидРасчетовСКонтрагентом = &ВидРасчетовСКонтрагентом"
			                            + " И УпрУчет = &УпрУчет)
						                |		";
			Запрос.УстановитьПараметр("ДоговорКонтрагента" + ТекНомер, СтрокаПлатежа.ДоговорКонтрагента);
			Запрос.УстановитьПараметр("Сделка" + ТекНомер, СтрокаПлатежа.Сделка);
			Запрос.УстановитьПараметр("ДокументРасчетовСКонтрагентом" + ТекНомер, СтрокаПлатежа.ДокументРасчетовСКонтрагентом);
			ТекНомер = ТекНомер + 1;
		КонецЕсли;
	...
	Если ТекстФильтра <> "" Тогда
		...
		ТекстФильтра = Сред(ТекстФильтра, 5);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстФильтра", ТекстФильтра);
		Запрос.Текст = ТекстЗапроса;
		...
		ОстаткиПоРасчетам.Индексы.Добавить("ДоговорКонтрагента, Сделка, ДокументРасчетовСКонтрагентом");
...
Процедура ОтражениеЗадолженностиВРегистреОперативныхРасчетовПоДокументам(СтруктураШапкиДокумента, ТаблицаВзаиморасчетов, ВидРасчетовСКонтрагентом, ВидДвижения, Движения, Отказ, Заголовок) Экспорт
	...
	|	ВзаиморасчетыСКонтрагентамиПоДокументамОстатки.УпрУчет,
	|	ВзаиморасчетыСКонтрагентамиПоДокументамОстатки.СуммаВзаиморасчетовОстаток * &ЗнакОстатка КАК СуммаВзаиморасчетов,
	|	ВзаиморасчетыСКонтрагентамиПоДокументамОстатки.СуммаРеглОстаток * &ЗнакОстатка КАК СуммаРегл
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов.Остатки(, &ТекстФильтра) КАК ВзаиморасчетыСКонтрагентамиПоДокументамОстатки
	...
	Для каждого СтрокаВзаиморасчетов Из ТаблицаВзаиморасчетов Цикл
		...
		Если ЭтоПогашениеКредДокумента Тогда
			...
			ТекстФильтра = ТекстФильтра + " ИЛИ (Организация = &Организация"
			                            + " И Контрагент = &Контрагент" 
			                            + " И ДоговорКонтрагента = &ДоговорКонтрагента" 
			                            + " И Сделка = &Сделка" + НомерСтрокой
			                            + " И ДокументРасчетовСКонтрагентом = &ДокументРасчетовСКонтрагентом" + НомерСтрокой
			                            + " И ВидРасчетовСКонтрагентом = &ВидРасчетовСКонтрагентом"
			                            + " И УпрУчет = &УпрУчет)
						                |		";
			Запрос.УстановитьПараметр("Сделка" + НомерСтрокой, СтрокаВзаиморасчетов.Сделка);
			Запрос.УстановитьПараметр("ДокументРасчетовСКонтрагентом" + НомерСтрокой, СтрокаВзаиморасчетов.ДокументРасчетовСКонтрагентом);
		КонецЕсли;
