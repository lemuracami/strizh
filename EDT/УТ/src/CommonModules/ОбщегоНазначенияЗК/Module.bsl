...
Функция ПоказыватьВалюту() 
	...
	Возврат Ложь;
	//ОпределитьНаличиеВалютныхПоказателей() Или ПроцедурыУправленияПерсоналом.ЗначениеУчетнойПолитикиПоПерсоналуВсехОрганизаций(глЗначениеПеременной("глУчетнаяПолитикаПоПерсоналуОрганизации"), "ИспользуютсяНачисленияВВалюте");
...
Функция ПолучитьЗначениеПеременной(ИмяПараметра, Кэш = Неопределено, КэшИзменен = Ложь) Экспорт
	...
	НайденноеЗначение = ОбщегоНазначенияЗКПереопределяемый.ПолучитьЗначениеПеременной(ИмяПараметра);
	...
	Если НайденноеЗначение = НеОпределено Тогда
		...
		ИначеЕсли ВРег(ИмяПараметра) = ВРег("УчетнаяПолитикаОтраженияЗарплатыВУчете") Тогда
			НайденноеЗначение = ОбщегоНазначенияЗКПереопределяемый.ЗаполнениеУчетнойПолитикиНалоговомуУчету();
		...
		ИначеЕсли ВРег(ИмяПараметра) = ВРег("глСпискиРегистраторов") Тогда
			НайденноеЗначение = ЗаполнитьСтруктуруРегистрРегистраторы();
			
		ИначеЕсли ВРег(ИмяПараметра) = ВРег("глЕстьВалютныеПоказателиОрганизаций") Тогда
