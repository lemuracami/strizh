...
//  ИмяТабличнойЧасти    - имя табличной части
//
Процедура НачалоВыбораЗначенияДокументаСоставногоТипаУпр(ДокументОбъект, ФормаДокумента, ЭлементФормы, 
	                   СтандартнаяОбработка, СтруктураОтбора, ИмяТабличнойЧасти,ОграничениеТипов = Неопределено) Экспорт
	...
	Для Каждого ЭлементСтруктуры Из СтруктураОтбора Цикл
		...
		Если ЭлементСтруктуры.Ключ = "ДоговорКонтрагента" Тогда
			ФормаВыбора.Список.Отбор.элементы[0].ПравоеЗначение = ЭлементСтруктуры.Значение;
			ФормаВыбора.Список.Отбор.элементы[0].Использование = Истина;
		//ИначеЕсли ЭлементСтруктуры.Ключ = "ДатаДокумента" Тогда
		//	ФормаВыбора.Список.Отбор.элементы[1].ПравоеЗначение = ЭлементСтруктуры.Значение;
		//	ФормаВыбора.Список.Отбор.элементы[1].Использование = Истина;
		КонецЕсли;
	КонецЦикла;
	//Если ЗначениеЗаполнено(ЭлементФормы.Значение) Тогда
	//	ФормаВыбора.ПараметрТекущаяСтрока = ЭлементФормы.Значение;
...
Функция ДатаКакМесяцПодобратьДатуПоТексту(Текст, ДатаПоТексту = НеОпределено) ЭКспорт
	...
	Если Найти(Текст, ".") <> 0 Тогда
		Подстроки = ОбщегоНазначенияЗК.РазложитьСтрокуВМассивПодстрок(Текст, ".");
	...
	Если Подстроки.Количество() = 1 Тогда
		...
		Если ОбщегоНазначенияЗК.ТолькоЦифрыВСтроке(Текст) Тогда
	...
	ИначеЕсли Подстроки.Количество() = 2 Тогда
		...
		Если ОбщегоНазначенияЗК.ТолькоЦифрыВСтроке(Подстроки[1]) Тогда
		...
		Иначе
			...
			Возврат СписокВозврата;
		КонецЕсли;                
		Если ЗначениеЗаполнено(Подстроки[0]) И ОбщегоНазначенияЗК.ТолькоЦифрыВСтроке(Подстроки[0]) Тогда
...
Процедура НачалоВыбораИзСпискаПредставленияПериодаРегистрации(Элемент, СтандартнаяОбработка, ПериодРегистрации, ЭтаФорма, НачальноеЗначение = Неопределено) Экспорт
	...
	Элемент = ДатаКакМесяцПредставление(ПериодРегистрации);
	//.Значение 
КонецПроцедуры
...
Функция ПериодПодобратьПоТексту(Текст, Периодичность, ДатаПоТексту = Неопределено) ЭКспорт
	...
	Если Подстроки.Количество() = 1 И Периодичность <> Перечисления.Периодичность.Год Тогда
		...
		Если ОбщегоНазначенияЗК.ТолькоЦифрыВСтроке(Текст) Тогда
			...
			Если ПериодЧислом >= 1 И ПериодЧислом <= КонецПериода Тогда
				Если Периодичность = Перечисления.Периодичность.День Тогда
					ДатаПоТексту = ОбщегоНазначенияЗК.ДобавитьИнтервал(НачалоМесяца, Периодичность, ПериодЧислом - 1);
				Иначе
					ДатаПоТексту = ОбщегоНазначенияЗК.ДобавитьИнтервал(НачалоГода, Периодичность, ПериодЧислом - 1);
				КонецЕсли;
	...
	Иначе
		Если Периодичность = Перечисления.Периодичность.День Тогда
			Если Подстроки.Количество() >= 2 И Не ПустаяСтрока(Подстроки[1]) И ОбщегоНазначенияЗК.ТолькоЦифрыВСтроке(Подстроки[1]) Тогда
			...
			Если Подстроки.Количество() >= 3 И Не ПустаяСтрока(Подстроки[2]) И ОбщегоНазначенияЗК.ТолькоЦифрыВСтроке(Подстроки[2]) Тогда
			...
			Если Подстроки.Количество() >= 1 И Не ПустаяСтрока(Подстроки[0]) И ОбщегоНазначенияЗК.ТолькоЦифрыВСтроке(Подстроки[0]) Тогда
		...
		ИначеЕсли Периодичность = Перечисления.Периодичность.Неделя Тогда
			...
			ДатаПоТексту = ОбщегоНазначенияЗК.ДобавитьИнтервал(Дата(Год, 1, 1), Периодичность, Неделя);
