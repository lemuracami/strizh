...
Функция ПолучитьУзелПланаОбмена(ID) Экспорт
	...
	Если Най.Пустая() Тогда
		...
		Нов.Код = СокрЛП(ID);
		Нов.ДатаСоздания = ТекущаяДатаСеанса();
		Нов.Записать();
		...
		ПланыОбмена.ЗарегистрироватьИзменения(Нов.Ссылка, Метаданные.Справочники.ТипыОплат);
...
Функция НеОбновлятьДанныеВMainshop() Экспорт
	Возврат Константы.НеОбновлятьДанныеВMainshop.Получить();
КонецФункции	
