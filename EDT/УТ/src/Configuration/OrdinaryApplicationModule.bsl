...
//БИТ Начало
Перем новаКонтекст Экспорт;


//Сборка мусора (освобождение памяти)
//
Процедура ВыполнитьСборкуМусора() Экспорт
	
	//новаКонтекст.СборщикМусора.ОчиститьСледующееРасширениеФормы();
	//новаКонтекст.СборщикМусора.ОчиститьСледующийОбъект();
...
Процедура ПриНачалеРаботыСистемы()
	...
	ОбновлениеРелиза.ВыполнитьОбновлениеНаНовыйРелиз();
	...
	//БИТ Начало
	новаКонтекст = Новый Структура;
	Если РольДоступна("новаАдминистраторУТЛ")
		Или РольДоступна("новаПользовательУТЛ") Или РольДоступна("Кассир") ИЛИ РольДоступна("ЧтениеПолное") Тогда //+++ Костя 031115 Добавил "Логист"
		
		новаУправлениеТранспортнойЛогистикой.Инициализация(новаКонтекст);
		//ПодключитьОбработчикОжидания("ВыполнитьСборкуМусора", 1);
		
		Если РольДоступна("новаАдминистраторУТЛ")
			Или РольДоступна("новаПользовательУТЛ") ИЛИ РольДоступна("Логист") ИЛИ РольДоступна("ЧтениеПолное") Тогда //+++ Костя 	 Тогда
			Если ИмяПользователя() <> "Склад" И ИмяПользователя() <> "Склад КБТ" И ИмяПользователя() <> "Склад стр 3" И ИмяПользователя() <> "Склад стр 2" Тогда
				ГлавныйИнтерфейс.новаМестнаяДоставка.Видимость = Истина;
			КонецеСли;	
		КонецЕсли	
	КонецЕсли;
	
	Если РольДоступна("СтаршийЛогист") Или РольДоступна("ЧтениеПолное") Тогда
		ГлавныйИнтерфейс.СтаршийЛогист.Видимость = Истина;
	КонецесЛИ;	
	
	Если РольДоступна("ВыдачаСдачаОборудования") Или РольДоступна("АдминистрированиеОборудования") Или РольДоступна("Админы") Тогда
		ГлавныйИнтерфейс.УчетОборудования.Видимость = Истина;
	КонецесЛИ;	
	...
	//Если РольДоступна("СтаршийЛогист") Тогда
	//	Если РольДоступна("ПолныеПрава") Тогда
	//		ГлавныйИнтерфейс.ПереключитьИнтерфейс("Полный, новаОбщий,новаМестнаяДоставка,СтаршийЛогист");
	//	Иначе	
	//		ГлавныйИнтерфейс.ПереключитьИнтерфейс("новаОбщий,новаМестнаяДоставка,СтаршийЛогист");
	//	КонецеСли;
	//КонецеСли;	
	...
	Попытка
		...
		Если ПараметрыСеанса.ЭтоТестоваяСреда Тогда
			//пСоед.СтрокаСоединения = "DRIVER={SQL Server};SERVER=85.93.93.15;UID=user1C;PWD=mallUser1Cpass_123!1;DATABASE=mainshop_test;LANGUAGE=русский";
			пСоед.СтрокаСоединения = "DRIVER={SQL Server};SERVER=192.168.5.141;UID=front_data_user;PWD=L8`R6Lng4Xnj]Zd-;DATABASE=mainshop_test;LANGUAGE=русский";
		Иначе	
			//пСоед.СтрокаСоединения = "DRIVER={SQL Server};SERVER=85.93.93.15;UID=user1C;PWD=mallUser1Cpass_123!1;DATABASE=mainshop;LANGUAGE=русский";
			пСоед.СтрокаСоединения = "DRIVER={SQL Server};SERVER=192.168.5.160;UID=front_data_user;PWD=L8`R6Lng4Xnj]Zd-;DATABASE=mainshop;LANGUAGE=русский";
		КонецеСли;	
		//пСоед.СтрокаСоединения = "DRIVER={SQL Server};SERVER=188.138.56.137;UID=user1C;PWD=mallUser1Cpass_123!;DATABASE=mainshop;LANGUAGE=русский";
		//пСоед.ИмяПользователя = "user1C";
		//пСоед.Пароль = "mallUser1Cpass_123!1";
		
		пСоед.ИмяПользователя = "front_data_user";
		пСоед.Пароль = "L8`R6Lng4Xnj]Zd-";
		
		псоед.СУБД = "MSSQLServer";
		аа=ВнешниеИсточникиДанных.ИнтернетМагазин;
		
		аа.УстановитьОбщиеПараметрыСоединения(пСоед);
		аа.УстановитьСоединение() ;	
	Исключение
	КонецПопытки;
	
	//если СокрЛП(ПараметрыСеанса.ТекущийПользователь.Наименование) = "Администратор УТЛ" Тогда
	Если РольДоступна("Админы") Тогда
		Если ЗначениеЗаполнено(Константы.БазаСД.Получить()) Тогда
			Если НЕ Константы.ЭтоТестоваяСреда.Получить() Тогда
				Сообщить("Подключено к базе: " + Константы.БазаСД.Получить() + ". РАБОЧАЯ СРЕДА", СтатусСообщения.Важное);
			КонецЕсли;	
		КонецеСли;
	КонецеСли;	
	
	Если НЕ Константы.ЭтоТестоваяСреда.Получить() Тогда
		ЗаголовокСистемы2 = " (" + СокрЛП(Константы.БазаСД.Получить()) + "). РАБОЧАЯ СРЕДА";
	Иначе
		ЗаголовокСистемы2 = " (" + СокрЛП(Константы.БазаСД.Получить()) + "). ТЕСТОВАЯ СРЕДА";
	КонецеСли;	
		
	УстановитьЗаголовокСистемы("УТЛ " + ЗаголовокСистемы2);
    
    //Серегин М.В. 10.08.2015 16:29:49 
    Если СокрЛП(мТекущийПользователь) = "ExchangeUTL" Тогда
        ОбработкаОбмена = Обработки.ЗагрузкаИзИнтернетМагазина.Создать();
        Форма = ОбработкаОбмена.ПолучитьФорму("Форма");
        Форма.Открыть();
        //ПрекратитьРаботуСистемы(Ложь); сеансы уходят в спящий режим и остаются не пойдет, но так и не понял почему
        //при запуске внешней обработки отрабатывает нормально
        ЗавершитьРаботуСистемы(Ложь);
    КонецЕсли;
    //Серегин М.В. 10.08.2015 16:29:52 
...
Процедура ПередЗавершениемРаботыСистемы(Отказ)
	...
	Если НЕ Отказ Тогда
		...
		// отдельно получаем настройки для которых нужно выполнить обмен при выходе из программы 1
	...
	Если Не Отказ Тогда
		Если РольДоступна("новаАдминистраторУТЛ")
			Или РольДоступна("новаПользовательУТЛ") ИЛИ РольДоступна("Логист") Тогда
			
			ОтключитьОбработчикОжидания("ВыполнитьСборкуМусора");
			
			Если новаКонтекст <> Неопределено Тогда
				
				Пока новаКонтекст.СборщикМусора.ОчиститьСледующийОбъект(Истина) Цикл
				КонецЦикла;
		...
		новаУправлениеТранспортнойЛогистикой.ПередЗавершениемРаботыСистемы(новаКонтекст);
...
Функция глПодключитьсяКДрайверуККТАтол() Экспорт
	Если глСерверАТОЛ = Неопределено Тогда
		Попытка
			ЗагрузитьВнешнююКомпоненту("C:\Program Files (x86)\ATOL\Drivers8\Bin_FREE\FPRNM1C.dll");
			глСерверАТОЛ = Новый("AddIn.FprnM45");
		исключение
			Сообщить("Ошибка загрузки внешней компоненты FPRNM1C.dll " + ОписаниеОшибки());
			Возврат -1;
		конецпопытки;
	КонецеСли;	
	
	Возврат глСерверАТОЛ;
КонецФункции	
