	...
	Для НомерЧасти = 1 По PartQuantity Цикл
		...
		Если НайтиФайлы(ИмяФайла).Количество() = 0 Тогда
			ШаблонСообщения = НСтр("ru = 'Не найден фрагмент %1 сессии передачи с идентификатором %2.
					...
					|КаталогВременныхФайловДляLinux и КаталогВременныхФайловДляWindows.'"
			);
...
Функция ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Иначе
		...
		Параметры.Добавить(ОбменДаннымиСервер.МассивИдентификаторовОбъектовМетаданных(Данные));
		
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбменДаннымиСервер.ЗарегистрироватьДанныеДляНачальнойВыгрузкиФоновымЗаданием",
											Параметры,
											,
...
Функция НайтиФайлЧасти(Знач Каталог, Знач НомерФайла)
	...
	Для КоличествоРазрядов = КоличествоРазрядовЦисла(НомерФайла) По 5 Цикл
...
Функция КоличествоРазрядовЦисла(Знач Число)
