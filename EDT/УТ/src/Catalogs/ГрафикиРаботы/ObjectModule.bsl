				...
				Пока ВыборкаПоВидуДня.Следующий() Цикл
					Если ВыборкаПоВидуДня.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Суббота Или 
					 ...
					 ИначеЕсли ВыборкаПоВидуДня.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Предпраздничный Тогда
						 РабочееВремя = РабочееВремя + ВыборкаПоВидуДня.КалендарныеДни * Макс(ДлительностьРабочейНедели / 5 - 1, 0);
						 РабочиеДни 	= РабочиеДни + ВыборкаПоВидуДня.КалендарныеДни;
					 КонецЕсли;
...
Процедура ПроверитьПравильностьЗаполненияПериодовСменыДляПятиШестидневки(Отказ, МассивОшибок = Неопределено)
	...
	Если ПериодыСмены.Количество() = 0 Тогда
		...
		Если МассивОшибок = Неопределено Тогда
			ОбщегоНазначенияЗК.СообщитьОбОшибке(ТекстОшибки, Отказ); 
		Иначе
...
Функция Печать(КалендарьНаборЗаписей, ГодПечати = Неопределено) Экспорт
	...
	Возврат УниверсальныеМеханизмыЗП.НапечататьДокумент(ПечатныйДокумент, , , "График работы " + Наименование,,ПечатныйДокумент);
...
Процедура АвтозаполнениеКалендаря(ДатаНачалаИнтервала, ДатаОкончанияИнтервала, СообщатьОбОшибках = Истина) Экспорт
	...
	Для инд = 0 по ДнейВИнтервале-1 Цикл
		...
		Если УчитыватьПраздники 
			И (НЕ ЭтоСменныйГрафик Или ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Праздник)
		...
		ИначеЕсли ЭтоПятиШестидневка Тогда
			...
			Если УчитыватьПраздники И КоличествоЧасовВсего <> 0 И ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Предпраздничный Тогда
				ЗаписьПоСменам.Часы	= Макс(КоличествоЧасовВсего - 1, 0);
			Иначе
				ЗаписьПоСменам.Часы	= КоличествоЧасовВсего;
			КонецЕсли;
