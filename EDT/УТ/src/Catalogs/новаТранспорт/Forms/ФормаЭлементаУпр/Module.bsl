...
// Заполнить данные пароля МП.
&НаСервере
Процедура ЗаполнитьДанныеПароляМП()
	Зап = Новый Запрос;
	Зап.Текст = 
	"ВЫБРАТЬ
	|	ПаролиТранспортаСрезПоследних.Пароль КАК Пароль
	|ИЗ
	|	РегистрСведений.ПаролиТранспорта.СрезПоследних(, Транспорт = &ВыбТранспорт) КАК ПаролиТранспортаСрезПоследних";
	Зап.УстановитьПараметр("ВыбТранспорт", Объект.Ссылка);
...
Процедура ЗаполнитьПромежуточныеЗначения()
	...
	ТекущийПарольМП = ПарольМП;
КонецПроцедуры

&НаКлиенте
Процедура ЗаписатьДанныеПароляТранспортаНаКлиенте()
	ЕСли ТекущийПарольМП = ПарольМП Тогда
	    Возврат;
	КонецеСли;	
	ЗаписатьДанныеПароляМП();
КонецПроцедуры

&НаСервере
Процедура ЗаписатьДанныеПароляМП()
	ТекДата = ТекущаяДатаСеанса();
	Наб = РегистрыСведений.ПаролиТранспорта.СоздатьНаборЗаписей();
	...
	НовЗапись.Транспорт = Объект.Ссылка;
	НовЗапись.Пароль = ПарольМП;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ЗаписатьДанныеПароляТранспортаНаКлиенте();
КонецПроцедуры
