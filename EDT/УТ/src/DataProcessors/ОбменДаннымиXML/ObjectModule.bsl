...
Перем мСоответствиеПравилКонвертации; // Соотвествие для определения правила конвертации объекта по типу этого объекта
...
Перем мСоответвиеТиповДанныхДляЗагрузки;
...
Перем мМенеджерРегистраСоответстствийОбъектов;
...
Функция ПолучитьПрефиксЧислоНомера(Знач Стр, ЧисловаяЧасть = "", Режим = "") Экспорт
	...
	СтрковыйНомерБезПрефикса = ПолучитьСтроковыйНомерБезПрефиксов(Стр);
	ДлинаСтроковойЧасти = СтрДлина(СтрковыйНомерБезПрефикса);
	Если ДлинаСтроковойЧасти > 0 Тогда
		ЧисловаяЧасть = Число(СтрковыйНомерБезПрефикса);
...
Функция ДобавитьКПрефиксу(Знач Стр, Добавок = "", Длина = "", Режим = "Слева",
	...
	СторкаЧисловойЧасти = Формат(ЧисловаяЧасть, "ЧГ=0");
	ДлинаЧисловойЧасти = СтрДлина(СторкаЧисловойЧасти);
	...
	Если ОбрезатьЧисловуюЧастьСлеваВСлучаеПревышенияНомера 
		...
		Если КоличествоСимволовУрезанияЧисловойЧасти > 0 Тогда
			СторкаЧисловойЧасти = Сред(СторкаЧисловойЧасти, КоличествоСимволовУрезанияЧисловойЧасти + 1);
		КонецЕсли;
		
	КонецЕсли;

	Результат = Результат + СторкаЧисловойЧасти;
...
Процедура УдалитьДвиженияУДокуемнта(ДокументОбъект, Отказ)
...
Процедура ЗарегистрироватьСоответствиеОбъектовДляОбмена(СсылкаНаОбъект, СвойстваПоиска)
	...
	// возможно есть запись с неуказанной ссылокй в другой ИБ. ЕЕ нужно удалить
...
Функция ОпределитьПоТипуОбъектаИспользоватьДополнительныйПоискПоПолямПоиска(ТипСсылкиСтрокой)
	...
	ЗначениеСоответстствия = мСоответствиеДопПараметровПоиска.Получить(ТипСсылкиСтрокой);
	
	Если ЗначениеСоответстствия <> Неопределено Тогда
		Возврат ЗначениеСоответстствия;
	КонецЕсли;
...
Функция ОпределитьПоТипуОбъектаАлгоритмПоследовательностиПолейПоиска(ТипСсылкиСтрокой, НастройкаПоиска)
	...
	ЗначениеСоответстствия = мСоответствиеАлгоритмовПоискаЭлементов.Получить(ТипСсылкиСтрокой);
...
// 
// Возвращаемое значение:
//  ЗначениеСоотвестствия = Правило конвертации объекта
// 
Функция ОпределитьПоТипуОбъектаПриемникаПравилоКонвертации(ТипСсылкиСтрокой)
	
	ЗначениеСоотвестствия = мСоответствиеПравилКонвертации.Получить(ТипСсылкиСтрокой);
...
Функция ОбработатьПоискОбъектаПоСтруктуре(НомерОбъекта, ТипОбъекта, СозданныйОбъект, 
	...
	Если СтруктураДанных <> Неопределено Тогда
		...
		Если СтруктураДанных.ИзвеснаяСсылка = Неопределено Тогда
			
			УстановитьСсылкуДляОбъекта(СтруктураДанных);
								
		КонецЕсли;
			
		СсылкаНаОбъект = СтруктураДанных.ИзвеснаяСсылка;
...
Процедура ПрочитатьИнформациюОСвойствахПоиска(ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать, 
	...
	ИнформацияОТипах = мСоответвиеТиповДанныхДляЗагрузки[ТипОбъекта];
...
Функция НайтиОбъектПоСсылке(ТипОбъекта, 
	...
	СвойствоСсылкаТекущегоЭлеменета = СвойстваПоиска["{КлючПоискаВИБПриемнике}"];
	...
	Если СвойствоСсылкаТекущегоЭлеменета <> Неопределено Тогда
		
		СсылкаНаОбъект = НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать, 
			ИмяТипаОбъекта, "{КлючПоискаВИБПриемнике}", СвойствоСсылкаТекущегоЭлеменета, ОбъектНайден, 
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, Имя, Очистить, ОбщаяИнформацияОТипеДокумента, НужноЗаписатьОбъект, 
	...
	Если Найти(Правило.ПоискПоТабличнымЧастям[0].поляпоискатч[0], ",") > 0 Тогда
		Стр = СтрЗаменить(Правило.ПоискПоТабличнымЧастям[0].поляпоискатч[0], ",", Символы.ПС);
		Мас = Новый Массив;
		Для Сч = 1 По СтрЧислоСтрок(Стр) Цикл
			Мас.Добавить(СтрПолучитьСтроку(Стр, Сч));
		КонецЦикла;	
		Мас.Добавить(Правило.ПоискПоТабличнымЧастям[0].поляпоискатч[1]);
		
		Правило.ПоискПоТабличнымЧастям[0].поляпоискатч = Мас;
	КонецЕСли;	
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	ИнформацияОТипах = мСоответвиеТиповДанныхДляЗагрузки[ТипСтрокиНабораРегистра];
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "СтрокиНабораЗаписей" Тогда
			...
			Если ФайлОбмена.ТипУзла <> одТипУзлаXML_КонецЭлемента Тогда
				...
				// срабатывет перед чтением первой записи в наборе
...
Функция ПроверитьНаличиеКоллизийИзмененийДляОбъекта(Объект, ИмяТипаОбъекта, ДополнительнаяИнформация = "", ПриоритетОбъектаОбмена)
	...
	Если НаличиеКоллизии Тогда
		...
		// если значения совпадают то коллизи нет
...
Процедура УстановитьСсылкуДляОбъекта(СтрокаСтекаНезаписанныхОбъектов)
	...
	СтрокаСтекаНезаписанныхОбъектов.ИзвеснаяСсылка = НоваяСсылка;
	
КонецПроцедуры

Процедура ДополнитьСтекНеЗаписанныхОбъектов(Нпп, ГНпп, Объект, ИзвеснаяСсылка, ТипОбъекта, ИмяТипа, АвтоматическиГенерироватьКод = Ложь, ПараметрыОбъекта = Неопределено)
	...
	мГлобальныйСтекНеЗаписанныхОбъектов.Вставить(НомерДляСтека, Новый Структура("Объект, ИзвеснаяСсылка, ТипОбъекта, ИмяТипа, АвтоматическиГенерироватьКод, ПараметрыОбъекта", 
		Объект, ИзвеснаяСсылка, ТипОбъекта, ИмяТипа, АвтоматическиГенерироватьКод, ПараметрыОбъекта));
...
Процедура _ВыгрузитьСубконто(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКС, 
	...
	Если ПКС.ПриводитьКДлине <> 0 Тогда
		...
		ВыполнитьПриведениеЗначенияКДлинне(Значение, ПКС);
...
Процедура ВыполнитьПриведениеЗначенияКДлинне(Значение, ПКС)
...
Функция ПолучитьЗапросИлиТаблицуДляВыборкиСвойств(СсылкаНаИсточник, ПКО, КоллекцияПКС, ПринудительноСоздатьЗапросДляПоискаНаКлиенте = Ложь)
	...
	ИначеЕсли Запрос = Неопределено Тогда
		...
		// для быстрого поиска все кешируем 
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
						   ...
						   НеИспользоватьПравилоСГлобальновВыгрузкойИНеЗапоминатьВыгруженные = Ложь) Экспорт
	...
	ЭтоПравилоСГлобальнойВыгрузкойОбъектов = (НеИспользоватьПравилоСГлобальновВыгрузкойИНеЗапоминатьВыгруженные = Ложь) 
		И ПКО.ИспользоватьБыстрыйПоискПриЗагрузке;
	
    ЗапоминатьВыгруженные       = (НеИспользоватьПравилоСГлобальновВыгрузкойИНеЗапоминатьВыгруженные = Ложь) И ПКО.ЗапоминатьВыгруженные;
			...
			Если СсылкаНаИсточник <> Неопределено
				...
				Иначе	
					...
					Если ТолькоПолучитьУзелСсылки Тогда
						...
						ВыгрузитьПоПравилу(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
								   ...
								   НеИспользоватьПравилоСГлобальновВыгрузкойИНеЗапоминатьВыгруженные);
	...
	Если Не НеНужноВыгружатьПоСоответвиюЗначений Тогда
		...
		Если УзелСсылки = Неопределено
			...
			//ПКЗ нужное свойсвто - тогда просто выгружаем пустую ссылку и все
...
Функция ОпределитьДляУзлаОбменаКоличествоЗарегистрированныхИзменений(УзелОбмена, Знач МаксимальноеКоличествоИнтересующихОбъектов = 0)
	...
	Для Каждого ЭлементСостава из МетаданныеПланаОбмена.Состав Цикл
		...
		КоличествоЗарегистрированныхЭлеменетов = ПолучитьКоличествоЗарегистрированныхОбъектовДляМетаданногоПоУзлу(МетаданныеДляОбмена, УзелОбмена);
...
Процедура ПровестиИзменениеРегистрацииДляСсылочногоОбъекта(ИмяТаблицыДляВыбора, КоличествоДляЗапроса, УзелИсточник, УзелПриемник, КоличествоРеальноПернесенныхОбъектов)
   ...
   |РегистрацияИзменений.Узел = &Узел";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если УдачноеИзменение Тогда
			КоличествоРеальноПернесенныхОбъектов = КоличествоРеальноПернесенныхОбъектов + 1;
		КонецЕсли;		
...
Процедура ПровестиИзменениеРегистрацииДляРегистра(ИмяТаблицыДляВыбора, МенеджерТипов, КоличествоДляЗапроса, УзелИсточник, УзелПриемник, 
	КоличествоРеальноПернесенныхОбъектов, НуженТолькоРегистратор)
...
Функция ИзменитьРегистрациюУОбъектовМетаданных(УзелИсточник, УзелПриемник, МетаданныеОбъекта, КоличествоОбъектовДляПереноса)
	...
	КоличествоРеальноПернесенныхОбъектов = 0;
	...
	ИначеЕсли ИмяТаблицыДляПоискаЗарегистрированных = "Справочник"
		...
		ПровестиИзменениеРегистрацииДляСсылочногоОбъекта(ИмяТаблицыДляПоискаЗарегистрированных + "." + МетаданныеОбъекта.Имя + ".Изменения", КоличествоДляЗапроса, УзелИсточник, УзелПриемник, КоличествоРеальноПернесенныхОбъектов);
	...
	ИначеЕсли ИмяТаблицыДляПоискаЗарегистрированных = "РегистрСведений" Тогда
		...
		ПровестиИзменениеРегистрацииДляРегистра(ИмяТаблицыДляПоискаЗарегистрированных + "." + МетаданныеОбъекта.Имя + ".Изменения", РегистрыСведений[МетаданныеОбъекта.Имя], КоличествоДляЗапроса, УзелИсточник, УзелПриемник, 
			КоличествоРеальноПернесенныхОбъектов, ЛОЖЬ);		
	...
	ИначеЕсли ИмяТаблицыДляПоискаЗарегистрированных = "РегистрНакопления"
		...
		ПровестиИзменениеРегистрацииДляРегистра(ИмяТаблицыДляПоискаЗарегистрированных + "." + МетаданныеОбъекта.Имя + ".Изменения", РегистрыНакопления[МетаданныеОбъекта.Имя], КоличествоДляЗапроса, УзелИсточник, УзелПриемник, 
			КоличествоРеальноПернесенныхОбъектов, Истина);
	...
	Возврат КоличествоРеальноПернесенныхОбъектов;
...
Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена, МассивПравилВыгрузкиДанных, 
	...
	Попытка
		...
		// и квитанции. Можно ограничить выборку данных только по метаданныем регистра, где хранится сопоставление объектов
...
Функция СоздатьСторкуСТипамиДляПриемника(СтруктураДанных)
...
Процедура ЗагрузитьИнформациюОТипахДанных()
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "ТипДанных" Тогда
			...
			мСоответвиеТиповДанныхДляЗагрузки.Вставить(Тип(ИмяТипа), СоответствиеТипа);
...
Процедура ЗагрузитьПравилаОбмена(Источник="", ТипИсточника="XMLФайл", СтрокаСообщенияОбОшибке = "", 
	...
	// дополинтельна нужна информация по типам данных п риемника для быстрой загрузки данных
	СтруктураДанных = Новый Соответствие();
	ЗаполнитьИнформациюПоТИпамданныхПриемника(СтруктураДанных, ТаблицаПравилКонвертации);
	
	мСтрокаТиповДляПриемника = СоздатьСторкуСТипамиДляПриемника(СтруктураДанных);
	...
	Попытка
		...
		Пока ФайлОбмена.Прочитать() Цикл
			...
			ИначеЕсли ИмяУзла = "УдалениеОбъектаПоСсылке" Тогда
				...
				// обработка удаления объекта из информационной базы
				СтрокаCсылкиУдаления = одАтрибут(ФайлОбмена, одТипСтрока, "СсылкаНаОбъект");
				
				Попытка
					
					Ссылка = ЗначениеИзСтрокиВнутр(СтрокаCсылкиУдаления);
...
Процедура ЗаписатьДанныеВПротоколОбменаЧерезCOMСоединение(СтруктураИстории) Экспорт
	...
	// выполняет запись информации об обмене данными чеерз COM соединение для информационной базы приемника
...
Функция ВыполнитьВыгрузку(НужноУдалитьПодключениеКИБПриемникуПослеВыгрузкиДанных = Истина, 
	...
	Исключение
		...
		ВыполнитьПередачуИнформацииПриЗавершанииПередачиДанных(Ложь);
...
Процедура ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеждерыДляПлановОбмена, ОбъектМД, ИмяТипа, Менеджер, ПрефиксИмениТипа, ВозможенПоискПоПредопределенным = Ложь)
...
Процедура ИнициализацияМенеджеров()
	...
	Для каждого ОбъектМД Из Метаданные.Справочники Цикл
		...
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеждерыДляПлановОбмена, ОбъектМД, "Справочник", Справочники[ОбъектМД.Имя], "СправочникСсылка", Истина);
	...
	Для каждого ОбъектМД Из Метаданные.ПланыВидовХарактеристик Цикл
		...
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеждерыДляПлановОбмена, ОбъектМД, "ПланВидовХарактеристик", ПланыВидовХарактеристик[ОбъектМД.Имя], "ПланВидовХарактеристикСсылка", Истина);
				
	КонецЦикла;
	
	Для каждого ОбъектМД Из Метаданные.ПланыСчетов Цикл
		
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеждерыДляПлановОбмена, ОбъектМД, "ПланСчетов", ПланыСчетов[ОбъектМД.Имя], "ПланСчетовСсылка", Истина);
						
	КонецЦикла;
	
	Для каждого ОбъектМД Из Метаданные.ПланыВидовРасчета Цикл
		
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеждерыДляПлановОбмена, ОбъектМД, "ПланВидовРасчета", ПланыВидовРасчета[ОбъектМД.Имя], "ПланВидовРасчетаСсылка", Истина);
...
Процедура ИнициализацияРеквизитовИМодульныхПеременных()
	...
	мМенеджерРегистраСоответстствийОбъектов = РегистрыСведений.СоответствиеОбъектовДляОбмена;
