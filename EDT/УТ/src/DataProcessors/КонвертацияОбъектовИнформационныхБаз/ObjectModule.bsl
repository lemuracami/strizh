...
Перем ПолеРежимСовместимостиСБСП20;
...
Функция ДокументыДляОтложенногоПроведения()
	...
	Если ТипЗнч(ПолеДокументыДляОтложенногоПроведения) <> Тип("ТаблицаЗначений") Тогда
		...
		ПолеДокументыДляОтложенногоПроведения.Колонки.Добавить("ДокументОбъект"); // Поле оставлено для совместимости с предыдущими версиями
...
Процедура ВыполнитьВыгрузкуДанных(ОбработкаДляЗагрузкиДанных = Неопределено) Экспорт
	...
	ПолеРежимСовместимостиСБСП20 = Неопределено;
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ, ИСПОЛЬЗУЕМЫЕ В ОБРАБОТЧИКАХ СОБЫТИЙ
...
Процедура ЗаписатьВФайлУдалениеОбъекта(Ссылка, Знач ТипПриемника, Знач ТипИсточника) Экспорт
	...
	УвеличитьСчетчикВыгруженныхОбъектов();
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Попытка
			...
			Иначе
				...
				ВыполнитьОбработчик_Конвертация_ПередКонвертациейОбъекта(ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные, ИмяПКО, ПКО,
...
Процедура УстановитьОбменДаннымиЗагрузка(Объект, Значение = Истина, Знач ОтправкаНазад = Ложь) Экспорт
	...
	Если Не ОтправкаНазад
		И УзелОбменаЗагрузкаДанных <> Неопределено
...
Процедура ЗагрузитьПКГС(ПравилаОбмена, ТаблицаСвойств, СвойстваОтключенные, СинхронизироватьПоИдентификатору, ИмяПКО = "")
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Свойство" Тогда
			...
			ЗагрузитьПКС(ПравилаОбмена, НоваяСтрока.ПравилаГруппы,, НоваяСтрока.ПравилаГруппыОтключенные, СтрокаПолейПоиска, СинхронизироватьПоИдентификатору, ИмяПКО, РодительПКС);
...
Процедура ЗагрузитьСвойства(ПравилаОбмена,
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Свойство" Тогда
			...
			ЗагрузитьПКС(ПравилаОбмена, ТаблицаСвойств, ТаблицаПоиска, СвойстваОтключенные,, СинхронизироватьПоИдентификатору, ИмяПКО);
...
Функция ПрочитатьОбъект(УникальныйИдентификаторСтрокой = "")
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = ТипУзлаXMLКонецЭлемента) Тогда
			...
			СтандартныеПодсистемыПереопределяемый.ПриПолученииДанныхОтПодчиненного(Неопределено, Объект, ПолучениеЭлемента, ОтправкаНазад);
					...
					Попытка
						...
						Если ЗаписатьОбъект Тогда
							...
							Если РежимЗаписи = РежимЗаписиДокумента.Запись Тогда
								...
								ЗаписатьОбъектВИБ(Объект, ТипОбъекта,, ОтправкаНазад);
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "ВерсияФормата" Тогда
			...
			Если РежимСовместимости <> Неопределено Тогда
				...
				ЗаписьXML.ЗаписатьАтрибут("РежимСовместимости", XMLСтрока(РежимСовместимости));
			КонецЕсли;
		...
		ИначеЕсли ИмяУзла = "РежимСовместимости" Тогда
			Значение = одЗначениеЭлемента(ПравилаОбмена, ТипСтрока);
			Конвертация.Вставить("РежимСовместимости",         Значение);
			одЗаписатьЭлемент(ЗаписьXML, ИмяУзла, Значение);
	...
	Если Не ПустаяСтрока(Конвертация.ПослеПолученияИнформацииОбУзлахОбмена) Тогда
			...
			Попытка
				...
				Иначе
					...
					ВыполнитьОбработчик_Конвертация_ПослеПолученияИнформацииОбУзлахОбмена(УзелОбменаЗагрузкаДанных);
...
Процедура ВыполнитьОтложенноеПроведениеДокументов()
	...
	ДокументыДляОтложенногоПроведения().Свернуть("ДокументСсылка, ДатаДокумента, ДокументПроведенУспешно");
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	Если СозданныеПриВыгрузкеОбъекты().Количество() > 0 Тогда
		...
		ПланыОбмена.ВыбратьИзменения(ЗаписьСообщения.Получатель, ЗаписьСообщения.НомерСообщения, СозданныеПриВыгрузкеОбъекты());
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Пока ВыборкаИзменений.Следующий() Цикл
		...
		Если ТипДанныхДляВыгрузки <> ТипРегистрСоответствия Тогда
			...
			СтандартныеПодсистемыПереопределяемый.ПриОтправкеДанныхПодчиненному(УзелДляОбменаОбъект, Данные, ОтправкаЭлемента, НачальнаяВыгрузкаДанных);
...
Процедура ИнициализироватьОбработки()
	...
	Если РежимОбмена = "Выгрузка" Или РежимОбмена = "" Тогда
		...
		Иначе
			...
			ИмяОбработкиВыгрузки = МенеджерПланаОбмена.ИмяОбработкиВыгрузки();
			
			Попытка
				
				ОбработкаВыгрузки = Обработки[ИмяОбработкиВыгрузки].Создать();
				
			Исключение
				
				СтрокаСообщенияОбОшибке = ЗаписатьВПротоколВыполнения(18);
				Возврат;
	...
	ИначеЕсли РежимОбмена = "Загрузка" Или РежимОбмена = "" Тогда
		...
		Иначе
			...
			ИмяОбработкиЗагрузки = МенеджерПланаОбмена.ИмяОбработкиЗагрузки();
			
			Попытка
				
				ОбработкаЗагрузки = Обработки[ИмяОбработкиЗагрузки].Создать();
...
Процедура СоздатьСтруктуруКонвертации()
	...
	Конвертация.Вставить("РежимСовместимости", "");
...
Процедура УвеличитьСчетчикВыгруженныхОбъектов()
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ ОБРАБОТЧИКОВ

// Возвращает интерфес обработчика во внешней обработке 
//
// Параметры:
// ИмяОбработки - Строка - Имя подключенной внешней обработки 
// ИмяОбработчика - Строка - Имя вызываемой процедуры
// ПараметрыОбработчика - Строка - Список параметров вызываемой процедуры
//
Функция ИнтерфейсОбработчика(ИмяОбработки, ИмяОбработчика, ПараметрыОбработчика)
	
	СтрокаВызова = "[ИмяОбработки].[ИмяОбработчика]([ПараметрыОбработчика]);";
	СтрокаВызова = СтрЗаменить(СтрокаВызова, "[ИмяОбработки]",  ИмяОбработки);
	СтрокаВызова = СтрЗаменить(СтрокаВызова, "[ИмяОбработчика]",	   ИмяОбработчика);
	СтрокаВызова = СтрЗаменить(СтрокаВызова, "[ПараметрыОбработчика]", ПараметрыОбработчика);
...
// Возвращает признак Режима совместимости
//
...
Процедура ВыполнитьОбработчик_Конвертация_ПослеЗагрузкиПравилОбмена()
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПослеЗагрузкиПравилОбмена;
	ПараметрыОбработчика = "";
	
	Выполнить(ИнтерфейсОбработчика(ИмяОбработки, ИмяОбработчика, ПараметрыОбработчика));
...
Процедура ВыполнитьОбработчик_Конвертация_ПередВыгрузкойДанных(ФайлОбмена, Отказ)
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПередВыгрузкойДанных;
	ПараметрыОбработчика = "ФайлОбмена, Отказ";
...
Процедура ВыполнитьОбработчик_Конвертация_ПередПолучениемИзмененныхОбъектов(Получатель, УзелДляФоновогоОбмена)
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПередПолучениемИзмененныхОбъектов;
	ПараметрыОбработчика = "Получатель, УзелДляФоновогоОбмена";
...
Процедура ВыполнитьОбработчик_Конвертация_ПослеВыгрузкиДанных(ФайлОбмена)
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПослеВыгрузкиДанных;
	ПараметрыОбработчика = "ФайлОбмена";
...
Процедура ВыполнитьОбработчик_Конвертация_ПередВыгрузкойОбъекта(ФайлОбмена, Отказ, ИмяПКО, Правило,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПередВыгрузкойОбъекта;
	ПараметрыОбработчика = "ФайлОбмена, Отказ, ИмяПКО, Правило, ВходящиеДанные, ИсходящиеДанные, Объект";
...
Процедура ВыполнитьОбработчик_Конвертация_ПослеВыгрузкиОбъекта(ФайлОбмена, Объект, ИмяПКО, ВходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПослеВыгрузкиОбъекта;
	ПараметрыОбработчика = "ФайлОбмена, Объект, ИмяПКО, ВходящиеДанные, ИсходящиеДанные, УзелСсылки";
	
	Выполнить(ИнтерфейсОбработчика(ИмяОбработки, ИмяОбработчика, ПараметрыОбработчика));
	
КонецПроцедуры

Процедура ВыполнитьОбработчик_Конвертация_ПередКонвертациейОбъекта(ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПередКонвертациейОбъекта;
	ПараметрыОбработчика = "ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные, ИмяПКО, ПКО,
	| ВыгруженныеОбъекты, Отказ, КлючВыгружаемыхДанных, ЗапоминатьВыгруженные, НеЗамещатьОбъектПриЗагрузке,
	| ВсеОбъектыВыгружены,ТолькоПолучитьУзелСсылки, Приемник, РежимЗаписи, РежимПроведения, НеСоздаватьЕслиНеНайден";
...
Процедура ВыполнитьОбработчик_Конвертация_ПередОтправкойИнформацииОбУдалении(Ссылка, Отказ)
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПередОтправкойИнформацииОбУдалении;
	ПараметрыОбработчика = "Ссылка, Отказ";
...
Процедура ВыполнитьОбработчик_Конвертация_ПередЗагрузкойДанных(ФайлОбмена, Отказ)
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПередЗагрузкойДанных;
	ПараметрыОбработчика = "ФайлОбмена, Отказ";
...
Процедура ВыполнитьОбработчик_Конвертация_ПослеЗагрузкиДанных()
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПослеЗагрузкиДанных;
	ПараметрыОбработчика = "";
...
Процедура ВыполнитьОбработчик_Конвертация_ПередЗагрузкойОбъекта(ФайлОбмена, Отказ, Нпп, Источник, ИмяПравила, Правило,
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПередЗагрузкойОбъекта;
	ПараметрыОбработчика = "ФайлОбмена, Отказ, Нпп, Источник, ИмяПравила, Правило, ГенерироватьНовыйНомерИлиКодЕслиНеУказан,
	| ТипОбъектаСтрокой, ТипОбъекта, НеЗамещатьОбъект, РежимЗаписи, РежимПроведения";
...
Процедура ВыполнитьОбработчик_Конвертация_ПослеЗагрузкиОбъекта(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта,
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПослеЗагрузкиОбъекта;
	ПараметрыОбработчика = "ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, ИмяТипаОбъекта, ОбъектНайден";
...
Процедура ВыполнитьОбработчик_Конвертация_ПриПолученииИнформацииОбУдалении(Объект, Отказ)
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПриПолученииИнформацииОбУдалении;
	ПараметрыОбработчика = "Объект, Отказ";		
...
Процедура ВыполнитьОбработчик_Конвертация_ПослеЗагрузкиПараметров(ФайлОбмена, Отказ, ПричинаОтказа)
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = "Конвертация_ПослеЗагрузкиПараметров";
	ПараметрыОбработчика = "ФайлОбмена, Отказ, ПричинаОтказа";
	
	Выполнить(ИнтерфейсОбработчика(ИмяОбработки, ИмяОбработчика, ПараметрыОбработчика));
	
КонецПроцедуры

Процедура ВыполнитьОбработчик_Конвертация_ПослеПолученияИнформацииОбУзлахОбмена(УзелОбменаЗагрузкаДанных)
	
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Конвертация.ИмяОбработчикаПослеПолученияИнформацииОбУзлахОбмена;
	ПараметрыОбработчика = "УзелОбменаЗагрузкаДанных";
...
Процедура ВыполнитьОбработчик_ПКО_ПередВыгрузкойОбъекта(ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКО.ИмяОбработчикаПередВыгрузкой;
	ПараметрыОбработчика = "ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные, ИмяПКО, ПКО, ВыгруженныеОбъекты,
	| Отказ, КлючВыгружаемыхДанных,ЗапоминатьВыгруженные, НеЗамещатьОбъектПриЗагрузке, ВсеОбъектыВыгружены,
	| ТолькоПолучитьУзелСсылки, Приемник, РежимЗаписи, РежимПроведения, НеСоздаватьЕслиНеНайден";
...
Процедура ВыполнитьОбработчик_ПКО_ПриВыгрузкеОбъекта(ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные, ИмяПКО, ПКО,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКО.ИмяОбработчикаПриВыгрузке;
	ПараметрыОбработчика = "ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные, ИмяПКО, ПКО, ВыгруженныеОбъекты,
	| КлючВыгружаемыхДанных, Отказ, СтандартнаяОбработка, Приемник, УзелСсылки";
...
Процедура ВыполнитьОбработчик_ПКО_ПослеВыгрузкиОбъекта(ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные, ИмяПКО, ПКО,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКО.ИмяОбработчикаПослеВыгрузки;
	ПараметрыОбработчика = "ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные,
	| ИмяПКО, ПКО, ВыгруженныеОбъекты, КлючВыгружаемыхДанных, Отказ, Приемник, УзелСсылки";
...
Процедура ВыполнитьОбработчик_ПКО_ПослеВыгрузкиОбъектаВФайлОбмена(ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные, ИмяПКО, ПКО,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКО.ИмяОбработчикаПослеВыгрузкиВФайл;
	ПараметрыОбработчика = "ФайлОбмена, Источник, ВходящиеДанные, ИсходящиеДанные,
	| ИмяПКО, ПКО, ВыгруженныеОбъекты, Приемник, УзелСсылки";
...
Процедура ВыполнитьОбработчик_ПКО_ПередЗагрузкойОбъекта(ФайлОбмена, Отказ, Нпп, Источник, ИмяПравила, Правило,
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПередЗагрузкой;
	ПараметрыОбработчика = "ФайлОбмена, Отказ, Нпп, Источник, ИмяПравила, Правило,ГенерироватьНовыйНомерИлиКодЕслиНеУказан,
	| ТипОбъектаСтрокой, ТипОбъекта,НеЗамещатьОбъект, РежимЗаписи, РежимПроведения";
...
Процедура ВыполнитьОбработчик_ПКО_ПриЗагрузкеОбъекта(ФайлОбмена, ОбъектНайден, Объект, НеЗамещатьОбъект, ОбъектМодифицирован, Правило)
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПриЗагрузке;
	ПараметрыОбработчика = "ФайлОбмена, ОбъектНайден, Объект, НеЗамещатьОбъект, ОбъектМодифицирован";
...
Процедура ВыполнитьОбработчик_ПКО_ПослеЗагрузкиОбъекта(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта,
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПослеЗагрузки;
	ПараметрыОбработчика = "ФайлОбмена, Отказ, Ссылка, Объект,
	| ПараметрыОбъекта, ОбъектМодифицирован, ИмяТипаОбъекта, ОбъектНайден";
...
Процедура ВыполнитьОбработчик_ПКО_ПоследовательностьПолейПоиска(НомерВариантаПоиска, СвойстваПоиска, ПараметрыОбъекта, ПрекратитьПоиск,
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ПараметрыОбработчика = "НомерВариантаПоиска, СвойстваПоиска, ПараметрыОбъекта, ПрекратитьПоиск,
	| СсылкаНаОбъект, УстанавливатьУОбъектаВсеСвойстваПоиска, СтрокаИменСвойствПоиска";
...
Процедура ВыполнитьОбработчик_ПКС_ПередВыгрузкойСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКС.ИмяОбработчикаПередВыгрузкой;
	ПараметрыОбработчика = "ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКС, ПКО,ОбъектКоллекции,
	| Отказ, Значение, ТипПриемника, ИмяПКО, ИмяПКОВидСубконто, Пусто, Выражение, УзелКоллекцииСвойств, НеЗамещать";
...
Процедура ВыполнитьОбработчик_ПКС_ПриВыгрузкеСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКС.ИмяОбработчикаПриВыгрузке;
	ПараметрыОбработчика = "ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	| ПКС, ПКО, ОбъектКоллекции, Отказ, Значение, КлючИЗначение, ВидСубконто, Субконто, Пусто,
	| ИмяПКО, ПКОСвойств,УзелСвойства, УзелКоллекцииСвойств,ИмяПКОВидСубконто";
...
Процедура ВыполнитьОбработчик_ПКС_ПослеВыгрузкиСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКС.ИмяОбработчикаПослеВыгрузки;
	ПараметрыОбработчика = "ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	| ПКС, ПКО, ОбъектКоллекции, Отказ, Значение, КлючИЗначение, ВидСубконто,Субконто, ИмяПКО,
	| ИмяПКОВидСубконто, ПКОСвойств, УзелСвойства, УзелСсылки, УзелКоллекцииСвойств, УзелСубконто";
...
Процедура ВыполнитьОбработчик_ПКГС_ПередОбработкойВыгрузки(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКГС.ИмяОбработчикаПередОбработкойВыгрузки;
	ПараметрыОбработчика = "ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	| ПКО, ПКГС, Отказ, КоллекцияОбъектов, НеЗамещать, УзелКоллекцииСвойств, НеОчищать";
...
Процедура ВыполнитьОбработчик_ПКГС_ПередВыгрузкойСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКГС.ИмяОбработчикаПередВыгрузкой;
	ПараметрыОбработчика = "ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	| ПКО, ПКГС, Отказ, ОбъектКоллекции, УзелКоллекцииСвойств, УзелКоллекцииОбъектов";
...
Процедура ВыполнитьОбработчик_ПКГС_ПриВыгрузкеСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКГС.ИмяОбработчикаПриВыгрузке;
	ПараметрыОбработчика = "ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС,
	| ОбъектКоллекции, УзелКоллекцииОбъектов, УзелОбъектаКоллекции, УзелКоллекцииСвойств, СтандартнаяОбработка";
...
Процедура ВыполнитьОбработчик_ПКГС_ПослеВыгрузкиСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКГС.ИмяОбработчикаПослеВыгрузки;
	ПараметрыОбработчика = "ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, 
	| ПКГС, Отказ, ОбъектКоллекции, УзелКоллекцииОбъектов, УзелКоллекцииСвойств, УзелОбъектаКоллекции";
...
Процедура ВыполнитьОбработчик_ПКГС_ПослеОбработкиВыгрузки(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = ПКГС.ИмяОбработчикаПослеОбработкиВыгрузки;
	ПараметрыОбработчика = "ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	| ПКО, ПКГС, Отказ, УзелКоллекцииСвойств, УзелКоллекцииОбъектов";
...
Процедура ВыполнитьОбработчик_ПВД_ПередОбработкойПравила(Отказ, ИмяПКО, Правило, ИсходящиеДанные, ВыборкаДанных)
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПередОбработкой;
	ПараметрыОбработчика = "Отказ, ИмяПКО, Правило, ИсходящиеДанные, ВыборкаДанных";
...
Процедура ВыполнитьОбработчик_ПВД_ПослеОбработкиПравила(ИмяПКО, Правило, ИсходящиеДанные)
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПослеОбработки;
	ПараметрыОбработчика = "ИмяПКО, Правило, ИсходящиеДанные";
...
Процедура ВыполнитьОбработчик_ПВД_ПередВыгрузкойОбъекта(ФайлОбмена, Отказ, ИмяПКО, Правило,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПередВыгрузкой;
	ПараметрыОбработчика = "ФайлОбмена, Отказ, ИмяПКО, Правило, ВходящиеДанные, ИсходящиеДанные, Объект";
...
Процедура ВыполнитьОбработчик_ПВД_ПослеВыгрузкиОбъекта(ФайлОбмена, Объект, ИмяПКО, ВходящиеДанные,
	...
	ИмяОбработки = "ОбработкаВыгрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПослеВыгрузки;
	ПараметрыОбработчика = "ФайлОбмена, Объект, ИмяПКО, ВходящиеДанные, ИсходящиеДанные, УзелСсылки";
...
Процедура ВыполнитьОбработчик_ПОД_ПередОбработкойПравила(Правило, Отказ, ИсходящиеДанные, ВыборкаДанных)
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПередОбработкой;
	ПараметрыОбработчика = "Правило, Отказ, ИсходящиеДанные, ВыборкаДанных";
...
Процедура ВыполнитьОбработчик_ПОД_ПередУдалениемОбъекта(Правило, Объект, Отказ, УдалитьНепосредственно, ВходящиеДанные)
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = Правило.ИмяОбработчикаПередУдалением;
	ПараметрыОбработчика = "Правило, Объект, Отказ, УдалитьНепосредственно, ВходящиеДанные";
...
Процедура ВыполнитьОбработчик_Параметры_ПослеЗагрузкиПараметра(Имя, Значение)
	...
	ИмяОбработки = "ОбработкаЗагрузки";
	ИмяОбработчика = "Параметры_" + Имя + "_ПослеЗагрузкиПараметра";
	ПараметрыОбработчика = "Имя, Значение";
