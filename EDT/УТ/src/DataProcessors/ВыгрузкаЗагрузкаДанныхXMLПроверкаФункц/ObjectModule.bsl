...
Процедура ВыполнитьВыгрузку(Знач ИмяФайла, ТолькоПроверкаНедопустимыхСимволов = Ложь) Экспорт
	...
	Иначе
		...
		ОповеститьОбОкончании("Выгрузка", ИмяФайла);
...
Процедура Инициализация() Экспорт
	...
	ДеревоМетаданных.Колонки.Добавить("ВыгружатьЗаПериодом", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Неотрицательный)));
...
Процедура СоставВыгрузки(ПересчитатьВыгружаемыеПоСсылке = Ложь) Экспорт
	...
	ЗаписатьЛогВыгрузки("Определен состав выгрузки " + СокрЛП(ТекущаяДата()));
...
Процедура ВыгрузкаДанных(ЗаписьXML, ТолькоПроверкаНедопустимыхСимволов = Ложь, ОбъектыВыгруженныеСОшибками = Неопределено)
	...
	Попытка
		...
		Для Каждого СтрокаТаблицыВыгрузки Из СоставПолнойВыгрузки Цикл
			...
			ЗаписатьЛогВыгрузки("Начало выгрузки " + СокрЛП(СтрокаДереваМетаданных.ЭлементОписания.ДляЗапроса) + " " + СокрЛП(СтрокаДереваМетаданных.ПолноеИмяМетаданных) + " " +  СокрЛП(ТекущаяДата()));
...
Функция ПодготовитьПостроительДляВыгрузки(СтрокаДереваМетаданных, СтрокаПолейДляВыборки = "") Экспорт
	...
	// -> Задача № 2478 Настройка запросов
	ЕстьДопУсловиеВыгрузки = Ложь;
	
	Если СтрокаДереваМетаданных.Родитель.Метаданные = "РегистрыСведений" Тогда
		
		Если СтрокаДереваМетаданных.ПолноеИмяМетаданных = "новаЗаданияРейсов"
			ИЛИ СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ПолигоныИКатегорииЗаказовДляРейсов"
			ИЛИ СтрокаДереваМетаданных.ПолноеИмяМетаданных = "УчетДанныхМППоРейсам" Тогда
			
			ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + "
			|  ГДЕ _.Рейс.Дата МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2"; 	
			ЕстьДопУсловиеВыгрузки = Истина;
		ИначеЕсли СтрокаДереваМетаданных.ПолноеИмяМетаданных = "новаРайоныАдресов" И новаРайоныАдресовПоПериоду Тогда
			
			
			///
			
			ИтоговыйТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			|  новаМестнаяДоставка.ТочкаПрибытия.Адрес КАК ТочкаПрибытияАдрес
			|  ПОМЕСТИТЬ ВТ_Доставки
			|  ИЗ
			|  	БизнесПроцесс.новаМестнаяДоставка КАК новаМестнаяДоставка
			|  ГДЕ
			|  	новаМестнаяДоставка.Дата МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2
			|  	И НЕ(новаМестнаяДоставка.ТочкаПрибытия ЕСТЬ NULL
			|  		ИЛИ новаМестнаяДоставка.ТочкаПрибытия = ЗНАЧЕНИЕ(Справочник.новаАдреса.ПустаяСсылка))
			|  ;
			|  
			|  ////////////////////////////////////////////////////////////////////////////////
			|  ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|  	новаРайоныАдресов.УказанВручную КАК УказанВручную,
			...
			|  		ПО ВТ_Доставки.ТочкаПрибытияАдрес = новаРайоныАдресов.Адрес
			|  	ГДЕ
			|  		НЕ новаРайоныАдресов.Классификатор ЕСТЬ NULL";
			
			ЕстьДопУсловиеВыгрузки = Истина;
			///
			
			
		ИначеЕсли СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ДополнительныеПараметрыЗаказа" 
			ИЛИ СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ИсторияРассылки" 
			ИЛИ СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ПозицииКВозврату" 
			ИЛИ СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ШтрихкодыЗаказов" Тогда
			
			ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + "
			|  ГДЕ _.Заказ.Дата МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2"; 	
			ЕстьДопУсловиеВыгрузки = Истина;

		ИначеЕсли СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ДополнительныеПараметрыЧеков" Тогда
			
			ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + "
			|  ГДЕ _.ЧекККМ.Дата МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2"; 	
			ЕстьДопУсловиеВыгрузки = Истина;
			
		ИначеЕсли СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ДополнительныеПараметрыЗабора" Тогда
			
			ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + "
			|  ГДЕ _.Забор.Дата МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2"; 	
			ЕстьДопУсловиеВыгрузки = Истина;
			
		ИначеЕсли СтрокаДереваМетаданных.ПолноеИмяМетаданных = "мгСоответствиеТочкиДоставкиЗоне" Тогда
	
			ИтоговыйТекстЗапроса = "
			|ВЫБРАТЬ
			|	новаМестнаяДоставка.ТочкаПрибытия КАК ТочкаПрибытия
			|ПОМЕСТИТЬ МестнаяДоставка
			|ИЗ
			|	БизнесПроцесс.новаМестнаяДоставка КАК новаМестнаяДоставка
			|ГДЕ
			|	новаМестнаяДоставка.Дата МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	_.ТочкаДоставки КАК ТочкаДоставки
			|ИЗ
			|	МестнаяДоставка КАК МестнаяДоставка
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.мгСоответствиеТочкиДоставкиЗоне КАК _
			|		ПО МестнаяДоставка.ТочкаПрибытия = _.ТочкаДоставки
			|	ГДЕ НЕ _.ТочкаДоставки Есть NULL";
			ЕстьДопУсловиеВыгрузки = Истина;
			
		ИначеЕсли СтрокаДереваМетаданных.ПолноеИмяМетаданных = "новаКалендарьВозможныхРейсов" Тогда
			
			ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + "
			|  ГДЕ _.НачалоПериода МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2"; 	
			ЕстьДопУсловиеВыгрузки = Истина;
			
		ИначеЕсли СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ШтрихкодыЗаказов" Тогда
			
			ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + "
			|  ГДЕ _.Заказ.Дата МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2"; 	
			ЕстьДопУсловиеВыгрузки = Истина;

		//ИначеЕсли СтрокаДереваМетаданных.ПолноеИмяМетаданных = "ВзаимосвязьДокументов" Тогда
		//	
		//	ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + "
		//	|  ГДЕ (_.Документ1 МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2)
		//	|	ИЛИ (_.Документ2 МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2)"; 	
		//	ЕстьДопУсловиеВыгрузки = Истина;
			
		КонецЕсли;
		
	ИначеЕсли	СтрокаДереваМетаданных.Родитель.Метаданные = "БизнесПроцессы" Тогда

			ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + "
			|  ГДЕ _.Дата МЕЖДУ //&ПараметрДата1 И //&ПараметрДата2"; 	
			ЕстьДопУсловиеВыгрузки = Истина;
			
	ИначеЕсли	СтрокаДереваМетаданных.Родитель.Метаданные = "Задачи" Тогда
	...
	Если ЕстьДопУсловиеВыгрузки Тогда
		
		ДатаНачалаСтрокой = СтрЗаменить(Год(ДатаНачала),Символы.НПП,"") + ", " + Месяц(ДатаНачала) + ", " + День(ДатаНачала) + ",00,00,00";
		ДатаОкончанияСтрокой = СтрЗаменить(Год(ДатаОкончания),Символы.НПП,"") + ", " + Месяц(ДатаОкончания) + ", " + День(ДатаОкончания) + ",23,59,59";
		
		ИтоговыйТекстЗапроса = СтрЗаменить(ИтоговыйТекстЗапроса, "//&ПараметрДата1", "ДАТАВРЕМЯ(" + ДатаНачалаСтрокой + ")");
		ИтоговыйТекстЗапроса = СтрЗаменить(ИтоговыйТекстЗапроса, "//&ПараметрДата2", "ДАТАВРЕМЯ(" + ДатаОкончанияСтрокой + ")");	
		...
		Если ЭтоЗапросПоОбъекту Тогда
			...
			Попытка
				...
				Если  ТипЗНЧ(Объект) = Тип("СправочникОбъект.Номенклатура") Тогда
					ПроверкаЗаписьXML = СоздатьОбъектЗаписиXMLДляПроверки();
					Попытка
						ЗаписатьXML(ПроверкаЗаписьXML, Объект);
					Исключение
						
						Объект.Артикул = ЗаменитьНедопустимыеСимволыXML(Объект.Артикул);
...
Процедура ЗаписатьЗначениеПриНеобходимости(АнализируемоеЗначение, ЗаписьXML, 
	...
	// Обмен данными УТЛ
	ЭтоДокумент = Ложь;
	ВыгрузитьВоЧтоБыТоНиСтало = Ложь;
	
	//Если СтрокаТаблицы <> Неопределено Тогда
	//	
	//	//Попытка 
	//	//	ЭтоДокумент = СокрЛП(СтрокаТаблицы.СтрокаДерева.Родитель.Метаданные) = "Документы"; // Метаданные.Документы.Содержит(АнализируемоеЗначение.Метаданные());
	//	//Исключение
	//	//Конецпопытки;
	//	//
	//	//Если ЭтоДокумент Тогда
	//	//	Если АнализируемоеЗначение.Дата < ДатаНачала Тогда //ИЛИ АнализируемоеЗначение.Дата > ДатаОкончания Тогда
	//	//		ВыгрузитьВоЧтоБыТоНиСтало = Истина
	//	//	КонецЕсли;	
	//	//КонецЕсли;	
	//	
	//КонецЕсли;
	...
	Если СтрокаТаблицы <> Неопределено  Тогда
		...
		// Обмен данными УТЛ
		Попытка
			Если СтрокаТаблицы.СтрокаДерева.ВыгружатьЗаПериодом = 1 Тогда
				
				Если (АнализируемоеЗначение.Дата < ДатаНачала) 
					ИЛИ (АнализируемоеЗначение.Дата > ДатаОкончания) Тогда
					
					ВыгрузитьВоЧтоБыТоНиСтало = Истина;
					
				КонецЕсли;	
				
				//Выгрузка форвард ссылок включена
			КонецЕсли;
		Исключение
		Конецпопытки;

		Если  НЕ ВыгрузитьВоЧтоБыТоНиСтало  Тогда
		// Обмен данными УТЛ
...
#Область ФильтрПодсистем

Процедура ЗаполнитьДеревоПодсистем()
	
	ДеревоПодсистем.Колонки.Очистить();
	ДеревоПодсистем.Колонки.Добавить("Выгружать", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Неотрицательный)));
	ДеревоПодсистем.Колонки.Добавить("Подсистема");
	ДеревоПодсистем.Колонки.Добавить("ПолныйПуть");
	ДеревоПодсистем.Колонки.Добавить("ИндексКартинки");

	Корень = ДеревоПодсистем.Строки.Добавить();
	Корень.Подсистема = "Общие";
	Корень.ИндексКартинки = 0;

	Для Каждого	Подсистема Из Метаданные.Подсистемы Цикл
		
		Если НЕ (Подсистема.Имя = "СистемноеНСИ" ИЛИ Подсистема.Имя = "ОбъектыКМиграцииПриОбрезкеБД") Тогда Продолжить КонецЕсли;
		
		СтрокаПодсистемы = Корень.Строки.Добавить();
		СтрокаПодсистемы.Подсистема = Подсистема;
		СтрокаПодсистемы.ПолныйПуть = Подсистема.Имя;
		СтрокаПодсистемы.ИндексКартинки = 1;
		ДобавитьПодчиненнуюПодсистему(Подсистема, СтрокаПодсистемы);
		
	КонецЦикла;	
	
КонецПроцедуры	

Процедура ДобавитьПодчиненнуюПодсистему(Подсистема, СтрокаПодсистемы)
	
	Для Каждого ПодчиненнаяПодсистема Из Подсистема.Подсистемы Цикл
		
		//Если ПодчиненнаяПодсистема.Имя = "РСКМиграции" 
		Если ПодчиненнаяПодсистема.Имя = "РНДвиженияКМиграции" 
			ИЛИ ПодчиненнаяПодсистема.Имя = "РСДокументовКМиграции"
			ИЛИ ПодчиненнаяПодсистема.Имя = "РНОстаткиКМиграции" Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаПодчиненнойПодсистемы = СтрокаПодсистемы.Строки.Добавить();
		СтрокаПодчиненнойПодсистемы.Подсистема = ПодчиненнаяПодсистема.Имя;
		СтрокаПодчиненнойПодсистемы.ПолныйПуть = СтрокаПодсистемы.ПолныйПуть + ".Подсистемы." + СокрЛП(ПодчиненнаяПодсистема.Имя);
		СтрокаПодчиненнойПодсистемы.ИндексКартинки = 1;
		ДобавитьПодчиненнуюПодсистему(ПодчиненнаяПодсистема, СтрокаПодчиненнойПодсистемы);
...
Процедура ЗаписатьЛогВыгрузки(Инфо)
	
	//Если ЭтотОбъект.ЛогироватьВЖурнал Тогда
		ЗаписьЖурналаРегистрации("Выгрузка данных в XML",УровеньЖурналаРегистрации.Информация,,,Инфо);
	//КОнецЕсли;

КонецПроцедуры

Процедура ЗаписатьЛогЗагрузки(Инфо)
	
	//Если ЭтотОбъект.ЛогироватьВЖурнал Тогда
		ЗаписьЖурналаРегистрации("Загрузка данных из XML",УровеньЖурналаРегистрации.Информация,,,Инфо);
	//КОнецЕсли;
...
Процедура ОповеститьОбОкончании(РежимРаботы, ИмяФайла = "") Экспорт
	
	МасП = Новый Массив;
	МасП.Добавить("rtoondra@yandex.ru");
	МасП.Добавить("r.drobnitsa@strizh-logistic.ru");
	МасП.Добавить("evgeniy.marochkin@strizh-logistic.ru");
	
	Если РежимРаботы = "Выгрузка" Или РежимРаботы = "Загрузка" Тогда
		ТекстСообщения = РежимРаботы + " в/из " + ИмяФайла  + " завершена " + СокрЛП(ТекущаяДата());
	Иначе
		ТекстСообщения = РежимРаботы + ". Обработка завершена " + СокрЛП(ТекущаяДата());
	КонецЕслИ;
	РезСтрукт = ОтправитьСообщение(МасП, РежимРаботы + " (обмен УТЛ)", ТекстСообщения,,"Стриж УТЛ");	
	Если Не РезСтрукт.Ответ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка отправки сообщения:
		|"+ТекстСообщения +". " + РезСтрукт.ОписаниеОшибки);
	КонецЕсли;	
...
Функция ОтправитьСообщение(СписокПолучателей = Неопределено, ТемаС = Неопределено, Знач ТекстС = Неопределено, ФайлС = Неопределено, ИмяОтправителя = "Логистическая компания ""Стриж""", ОбратныйАдрес = "mind@strizh-logistic.ru", ТипТекста = Неопределено, Сообщение = Неопределено, Ошибка = "", ТипРассылки = Неопределено, ДополнитьАдреса = Истина) Экспорт
	...
	//П.ПользовательSMTP = "r.drobnitsa@strizh-logistic.ru"; 
	//П.ПарольSMTP = "V0qA3o9q";
	//ИмяОтправителя = "Оповещение переноса УТЛ";
	//ОбратныйАдрес = "r.drobnitsa@strizh-logistic.ru";

	//П.ПользовательSMTP = "mind@strizh-logistic.ru"; //mind
	П.ПользовательSMTP = "noreply2@strizh-logistic.ru";//noreply2
	//П.ПарольSMTP = "pzW5z6Vu";//mind
	П.ПарольSMTP = "kjaelrj985";//noreply2
	ИмяОтправителя = "Оповещение переноса УТЛ";
...
Функция ТекстЗапросаПоПаттернам(ПаттерныЗапроса, ТолькоПоДатеНачала = Истина, ДатаВЗапросаПоНачалу = Ложь, СсылкиЗапросаЗаПериодом = Ложь) Экспорт
		
	ТекстЗапроса = "";
	ИмяСсылки = " КАК Ссылка";
	СтрокаВТ = " КАК ТипСсылки
	|ПОМЕСТИТЬ ВТ_Ссылки";
	
	Меньше = ?(СсылкиЗапросаЗаПериодом,"<", "<=");
	Больше = ?(СсылкиЗапросаЗаПериодом,">", ">=");
		
	Для Каждого Паттерн Из ПаттерныЗапроса Цикл
		
		ОписаниеЗапроса = Паттерн.Значение;

		РеквизитОбъектаВыгрузки = ОписаниеЗапроса.РеквизитЗапроса;
		РеквизитВыгрузкиБезПериода = Ложь;
		Если Найти(РеквизитОбъектаВыгрузки, "%ОбъектДляВыгрузкиБезПериода") > 0 Тогда
			СоставРеквизита = СтрРазделить(РеквизитОбъектаВыгрузки,"%");
			РеквизитОбъектаВыгрузки = СоставРеквизита[0];
			РеквизитВыгрузкиБезПериода = Истина;
		КонецЕсли;
		...
		Если Лев(ОписаниеЗапроса.ОбъектЗапроса, 11) = "Справочник." Тогда
			ТекстПаттерна = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			| 	_Объект." + ОписаниеЗапроса.РеквизитЗапроса + ИмяСсылки + ",
			|	ТИПЗНАЧЕНИЯ(_Объект."+ ОписаниеЗапроса.РеквизитЗапроса +") "+ СтрокаВТ  + "
			|ИЗ  " + ОписаниеЗапроса.ОбъектЗапроса + " КАК _Объект";
			
		ИначеЕсли Лев(ОписаниеЗапроса.ОбъектЗапроса, 16) = "РегистрСведений." Тогда
			
			ВедущийРеквизит = ОписаниеЗапроса.ВедущийРеквизит;
			Если Не ВедущийРеквизит = "Период" Тогда 
				ВедущийРеквизит = ВедущийРеквизит  + ".Дата";
			КонецЕсли;	
			
			ТекстПаттерна = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			| 	_Объект." + РеквизитОбъектаВыгрузки + ИмяСсылки + ",
			|	ТИПЗНАЧЕНИЯ(_Объект."+ РеквизитОбъектаВыгрузки +") "+ СтрокаВТ  + "
			|ИЗ  " + ОписаниеЗапроса.ОбъектЗапроса + " КАК _Объект
			|ГДЕ _Объект." + ВедущийРеквизит + " Между &ДатаНачала И &ДатаОкончания";			
		...
		Иначе
			...
			|	ТИПЗНАЧЕНИЯ(_Объект."+ ОписаниеЗапроса.РеквизитЗапроса +") "+ СтрокаВТ  + "
			|ИЗ  " + ОписаниеЗапроса.ОбъектЗапроса + " КАК _Объект
			|ГДЕ _Объект.Ссылка.Дата Между &ДатаНачала И &ДатаОкончания";
			
		КонецЕсли;
		ИмяСсылки = "";
		СтрокаВТ = "";
		
		Если Не РеквизитВыгрузкиБезПериода Тогда
			Если ДатаВЗапросаПоНачалу Тогда
				Если ТолькоПоДатеНачала Тогда
					ТекстПаттерна = ТекстПаттерна + "
					|	И _Объект." + ОписаниеЗапроса.РеквизитЗапроса + ".Дата "+Меньше+" &ДатаНачалаПериода
					|";	
				Иначе
					ТекстПаттерна = ТекстПаттерна + "
					|	И (_Объект." + ОписаниеЗапроса.РеквизитЗапроса + ".Дата "+Меньше+" &ДатаНачалаПериода ИЛИ _Объект." + ОписаниеЗапроса.РеквизитЗапроса + ".Дата "+ Больше +" &ДатаОкончания)
					|";	
				КонецЕсли;
			Иначе
				Если ТолькоПоДатеНачала Тогда
					ТекстПаттерна = ТекстПаттерна + "
					|	И _Объект." + ОписаниеЗапроса.РеквизитЗапроса + ".Дата "+Меньше+" &ДатаНачала
					|";	
				Иначе
					ТекстПаттерна = ТекстПаттерна + "
					|	И (_Объект." + ОписаниеЗапроса.РеквизитЗапроса + ".Дата "+Меньше+" &ДатаНачала ИЛИ _Объект." + ОписаниеЗапроса.РеквизитЗапроса + ".Дата "+ Больше +" &ДатаОкончания)
					|";	
				КонецЕсли;				
			КонецЕслИ;	
		КонецЕслИ;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_Ссылки.Ссылка КАК Объект,
	//|	ВТ_Ссылки.Ссылка.Номер КАК Номер,
	|	ВТ_Ссылки.ТипСсылки КАК ТипСсылки
	|ИЗ
