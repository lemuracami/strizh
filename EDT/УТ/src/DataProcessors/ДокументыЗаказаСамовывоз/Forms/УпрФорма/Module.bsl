...
Процедура ОчиститьДанные()
	...
	//+Широков Задача 4495 11.03.2021
	//Объект.Клиент = Справочники.Контрагенты.ПустаяСсылка();
	Объект.Клиент = Справочники.ПолучателиЗаказов.ПустаяСсылка();
	//-Широков Задача 4495 11.03.2021
...
Функция ПолучитьДокументы(Номер)
	...
	//            |	РеализацияТоваровУслуг.ВладелецТовара.Ссылка КАК Параметр3,
	...
	//            |	новаЗаданияРейсов.Рейс.Транспорт.НомерЛог";
		...
		Зап.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		            ...
		            |	РейсЗаказы.Ссылка.Номер КАК Номер,
		            ...
		            |ГДЕ
		            |	РейсЗаказы.Заказ.Номер = &Номер
		            |	И РейсЗаказы.Заказ ССЫЛКА Документ.РеализацияТоваровУслуг
		            |
		            |УПОРЯДОЧИТЬ ПО
		            |	РейсЗаказы.Ссылка.ДатаРейса УБЫВ
		            |;
		            |
		            |////////////////////////////////////////////////////////////////////////////////
		            |ВЫБРАТЬ
		            ...
		            |	РеализацияТоваровУслуг.Номер КАК Параметр4,
		            |	РеализацияТоваровУслуг.НомерВнешнегоЗаказа КАК Параметр5,
		            |	NULL КАК НомерПалетты,
		            |	NULL КАК Водитель,
		            |	NULL КАК Экспедитор,
		            |	ДополнительныеПараметрыЗаказа.КодИдентификацииКлиента КАК КИК
		            |ИЗ
		            |	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
		            |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ТипыОплат КАК ТипыОплат
		            |		ПО РеализацияТоваровУслуг.ТипОплаты = ТипыОплат.Код
		            ...
		            |	ТелефонныеАппаратыТранспортаСрезПоследних.ТА.Ссылка,
		            |	новаЗаданияРейсов.Рейс.Транспорт.НомерЛог,
		            |	ЕСТЬNULL(втРейсы.Номер, """"),
		            |	NULL,
		            |	ЕСТЬNULL(втРейсы.НомерПалетты, 0),
		            |	ЕСТЬNULL(ПривязкаМашинКРейсамСрезПоследних.Водитель.Представление, """"),
		            |	ЕСТЬNULL(ПривязкаМашинКРейсамСрезПоследних.Экспедитор.Представление, """"),
		            |	NULL
		            |ИЗ
		            |	РегистрСведений.новаЗаданияРейсов КАК новаЗаданияРейсов
		            |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТелефонныеАппаратыТранспорта.СрезПоследних(&ДатаЗапроса, ) КАК ТелефонныеАппаратыТранспортаСрезПоследних
		            ...
		            |			ПО втРейсы.Рейс = ПривязкаМашинКРейсамСрезПоследних.Рейс
		            |		ПО новаЗаданияРейсов.Рейс = втРейсы.РейсМестнойДоставки
		            |ГДЕ
		            |	новаЗаданияРейсов.Доставка.Номер = &Номер
		            |	И новаЗаданияРейсов.Рейс В
		            |			(ВЫБРАТЬ
		            |				втРейсы.РейсМестнойДоставки
		            |			ИЗ
		            |				втРейсы КАК втРейсы)
		            |
		            |СГРУППИРОВАТЬ ПО
	...
	Для Каждого Тек Из Рез Цикл
		...
		Если ТипЗнч(Тек.Документ) = Тип("БизнесПроцессСсылка.новаРейсМестнойДоставки") Тогда
			//+Степанов Задача № 3487
			Объект.НомерПалетты = Тек.НомерПалетты;
			Объект.НомерРейса   = Тек.Параметр4;
			Объект.Водитель = Тек.Водитель;
			Объект.Экспедитор = Тек.Экспедитор;
			//-Степанов Задача № 3487
...
Функция СтатусЗаказа(Номер)
	...
	//КонецеСли;	
	Если Объект.Реализация.Пустая() Тогда //Объект.Доставка.Пустая() Тогда
		Возврат Неопределено;
	КонецеСли;		
...
Процедура ДокументыСписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ВыбраннаяСтрока <> Неопределено Тогда
		...
		Если ТЗ = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			...
			Или ТЗ = Тип("ДокументСсылка.СнятиеДоставокСПланирования")Тогда			
...
Функция ПолучитьТабличныйДокументЗаказа(Таб, Доставка, Номер, МасДос)
	...
	//CeHbKA 03.06.2019
	Печ = Обработки.ПечатьБланковЗаказовНовый.Создать();
	Печ.ПечатьПоДоставке2(Доставка, Таб, Истина);
	//CeHbKA 03.06.2019
	Возврат Таб;
КонецФункции	
...
Функция ПолучитьТабличныйДокументЗаказаВыбор(Таб, Доставка, Номер, МасДос)
	...
	Печ = Обработки.ПечатьБланковЗаказовНовый.Создать();
	//Печ.ПечатьПоДоставке(Доставка, Таб, Истина);
	//CeHbKA 03.06.2019
	Печ.ПечатьПоДоставке2(Доставка, Таб, Истина);	
	//CeHbKA 03.06.2019
	// <<--- Михушкин 
...
Процедура СписокДоковВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ВыбраннаяСтрока <> Неопределено Тогда
		...
		Если ТЗ = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			...
			Или ТЗ = Тип("ДокументСсылка.ДоставкиСОтклонениямиКоличестваСписком") Тогда
			Ф = Д.ПолучитьФорму("ФормаДокумента");
			если ТЗ = Тип("ДокументСсылка.ЗакрытиеЗаказов") Тогда
				 Ф = Д.ПолучитьФорму("ФормаДокументаСПоказателями");
			КонецеСли;	 
...
Процедура ОтправитьСМС(Команда)
		...
		Сендер.ШаблонРассылки = Справочники.ШаблоныСМСРассылок.НайтиПоКоду("000000019");
		...
		Ф.ОтправитьСМССамовывоз();
КонецПроцедуры


&НаКлиенте
Процедура ВводДанныхОСостоянииЗаказа(Команда)
	// Вставить содержимое обработчика.
КонецПроцедуры
...
&НаКлиенте
Процедура НапечататьЗаказ_СтарыйБланк(Команда)
	...
	Иначе
		...
		КонецЦикла;	
		Если Д <> Неопределено Тогда
			ПолучитьТабличныйДокументЗаказаВыбор_Старый(Т, Д, СокрЛП(Д.Номер), 0);
			Т.Показать();
		КонецеСлИ;
	КонецеСли;
...
&НаКлиенте
Функция ПолучитьТабличныйДокументЗаказаВыбор_Старый(Таб, Доставка, Номер, МасДос)
	//Печ = Обработки.ПечатьБланковЗаказов.Создать();
	//Печ.ВыборПечати(Таб, Доставка, Номер, масДос, Неопределено);
	// Михушкин --->> 
	//БПДоставка = БизнесПроцессы.новаМестнаяДоставка.НайтиПоНомеру(СокрЛП(Доставка.Номер));
	Печ = Обработки.ПечатьБланковЗаказовНовый.Создать();
	Печ.ПечатьПоДоставке_старая(Доставка, Таб, Истина);
	// <<--- Михушкин 
	Возврат Таб;
КонецФункции