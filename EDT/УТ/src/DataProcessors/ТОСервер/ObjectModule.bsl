		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			...
			Для Каждого Позиция Из ОписаниеЧека Цикл
				...
				Если СуммаСоСкидкой1С = СуммаСоСкидкойФР Тогда
					Если Объект.Модель = "ШТРИХ-ФР-К" Тогда
...
//  <ПеречислениеСсылка.ТООшибки*> - Результат работы функции
//
Функция ОтчетСГашением(Идентификатор, Пароль, НомерЧека, НомерСмены) Экспорт
...
Функция ОбработатьСобытиеСШК(Обработка, Объект, Клиент, Событие, Данные, СШК)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Иначе // Если ТипЗнч(РезультатЗапроса.Владелец) = Тип("СправочникСсылка.Номенклатура")
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Лев(ШК, 2) = "37" Тогда
		ЗапДоставки = Новый Запрос;
		ЗапДоставки.Текст = 	"ВЫБРАТЬ
		                    	...
		                    	|	РеализацияТоваровУслуг.ТранспортнаяКомпания,
		                    	|	ВТРейсы.Рейс,
		                    	|	ВТРейсы.Транспорт,
		                    	|	ВТРейсы.Водитель,
		                    	|	ВТРейсы.Экспедитор
		                    	...
		                    	|ГДЕ
		                    	|	РеализацияТоваровУслуг.Номер = &ВыбНом
		                    	|
		                    	|УПОРЯДОЧИТЬ ПО
		                    	|	РеализацияТоваровУслуг.Дата,
		                    	|	ВТРейсы.Рейс.Дата УБЫВ
		                    	|;
		                    	|
		                    	|////////////////////////////////////////////////////////////////////////////////
		                    	|УНИЧТОЖИТЬ ВТРейсы";
								ШК_ = Формат(Число(Сред(ШК, 3, 10)), "ЧГ=");
						ЗапДоставки.УстановитьПараметр("ВыбНом", ШК_);
						ЗапДоставки.УстановитьПараметр("ТипЗаказа", Перечисления.ТипыЗаказов.Доставка);
						ЗапДоставки.УстановитьПараметр("НаДату", ТекущаяДата());
						Рез = ЗапДоставки.Выполнить().Выгрузить();
						
						Ответ = Ложь;
						Если Рез.Количество() > 0 Тогда
							Ответ = Клиент.СШКЗаказы(Рез[0]);
						КонецеСли;	
						Если Ответ Тогда
							Возврат Результат;
						КонецЕсли;
	ИначеЕсли Лев(ШК, 2) = "38" Тогда
		ЗапДоставки = Новый Запрос;
		ЗапДоставки.Текст = 	"ВЫБРАТЬ
		                    	|	РеализацияТоваровУслуг.Ссылка КАК Реализация,
		                    	|	РеализацияТоваровУслуг.СтатусИнтернетМагазина КАК СтатусЗаказа,
		                    	...
		                    	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РасчетКассовогоОбслуживания.СрезПоследних(&НаДату, ТипЗаказа.Ссылка = &ТипЗаказа) КАК РасчетКассовогоОбслуживанияСрезПоследних
		                    	|		ПО РеализацияТоваровУслуг.ВладелецТовара.Ссылка = РасчетКассовогоОбслуживанияСрезПоследних.Контрагент.Ссылка,
		                    	|	РегистрСведений.СостоянияЗаказов.СрезПоследних(&НаДату, ) КАК СостоянияЗаказовСрезПоследних
		                    	|ГДЕ
								...
								ШК_ = Формат(Число(Сред(ШК, 3, 10)), "ЧГ=");
						ЗапДоставки.УстановитьПараметр("ВыбНом", ШК_);
						ЗапДоставки.УстановитьПараметр("ТипЗаказа", Перечисления.ТипыЗаказов.Самовывоз);
						ЗапДоставки.УстановитьПараметр("НаДату", ТекущаяДата());
						Рез = ЗапДоставки.Выполнить().Выгрузить();
						...
						Если Ответ Тогда
							Возврат Результат;
						КонецЕсли;						
	ИначеЕсли Лев(ШК, 2) = "31" Тогда
		НомРейса = Формат(Число(Сред(Шк, 3, 10)), "ЧЦ=9; ЧВН=; ЧГ=");
		НайРейс = БизнесПроцессы.новаРейсМестнойДоставки.НайтиПоНомеру(НомРейса);
		
		Ответ = Клиент.СШКРейс(НайРейс);
		Если Ответ Тогда
			Возврат Результат;
		КонецЕсли;
	ИначеЕсли СтрДлина(ШК) = 16 Тогда	
		Ном = Прав(Шк, 13);
		
		Ответ = Ложь;
		Най = Документы.РеализацияТоваровУслуг.НайтиПоРеквизиту("НомерВнешнегоЗаказа", Ном);
		Если Не Най.Пустая() Тогда
			Най2 = Документы.ПоступлениеТоваровУслуг.НайтиПоНомеру(Най.Номер);  //, Най.Дата
			Ответ = Клиент.СШКЗаказы(Най2);
		Иначе
			Ном = Прав(Шк, 8);
	...
	ИначеЕсли СтрДлина(ШК) = 6 Тогда	
		...
		ЗапДоставки.Текст = 	"ВЫБРАТЬ
								...
								ШК_ = ШК;
						ЗапДоставки.УстановитьПараметр("Номер", ШК);
						Рез = ЗапДоставки.Выполнить().Выгрузить();
						
						Если Рез.Количество() > 0 Тогда
							Ответ = Клиент.СШКЗаказы(Рез[0].Поступление);
						КонецеСли;	
						Если Ответ Тогда
							Возврат Результат;
						КонецЕсли;						
