...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = НСтр("ru = 'Универсальный обмен данными в формате XML ('") + ВерсияОбъектаСтрокойНаСервере() + ")";
...
Процедура ТестПодключения(Команда)
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		Предупреждение(НСтр("ru = 'Подключение к информационной базе приемнику произошло успешно.'"));
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		Если Не ПоместитьФайл(АдресФайла,"Файл обмена",,,ЭтаФорма.УникальныйИдентификатор) Тогда
			Возврат;
		КонецЕсли;
...
Процедура НастройкаОтладкиВыгрузки(Команда)
	...
	#Если ВебКлиент Тогда
		Предупреждение(НСтр("ru = 'Возможности отладки недоступны при использовании Веб-клиента'"));
		Возврат;
	#КонецЕсли
	
	Если Не ПравилаЗагружены Тогда
		
		Предупреждение(НСтр("ru = 'Необходимо прочитать правила обмена'"));
		Возврат;
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		Предупреждение(НСтр("ru = 'Выберите файл обмена данными.'"));
		Если Не ПоместитьФайл(АдресФайлаОбменаВХранилище, "Файл обмена",ИмяФайлаДляРасширения,,ЭтаФорма.УникальныйИдентификатор) Тогда
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если ПустаяСтрока(ИмяФайлаОбмена) Тогда
			Предупреждение(НСтр("ru = 'Не задан файл обмена данными.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		Если Не ПоместитьФайл(АдресФайлаПравилВХранилище, "Файл правил обмена", ИмяФайлаДляРасширения,,ЭтаФорма.УникальныйИдентификатор) Тогда
			Возврат;
		КонецЕсли;
...
// 
&НаКлиенте
Процедура ОткрытьВПриложении(ТекстРедактирования, СтандартнаяОбработка)

	ЗапуститьПриложение("explorer " + ТекстРедактирования);
...
Функция ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		Сообщить(НСтр("ru = 'Подключение к информационной базе приемнику произошло успешно.'"));
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			Сообщить(НСтр("ru = 'Не найден файл обмена на сервере.'"));
			Возврат;
...
Процедура ВыполнитьЗагрузкуИзФормы()
	...
	Перем АдресФайла, ИмяФайлаДляРасширения;
	...
	Если ЭтоКлиент Тогда
		...
		Предупреждение(НСтр("ru = 'Выберите файл обмена данными.'"));
		Если Не ПоместитьФайл(АдресФайла, "Файл обмена", ИмяФайлаДляРасширения,,ЭтаФорма.УникальныйИдентификатор) Тогда
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если ПустаяСтрока(ИмяФайлаОбмена) Тогда
			Предупреждение(НСтр("ru = 'Выберите файл обмена данными на сервере.'"));
			Возврат;
		КонецЕсли;
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не ПрямаяВыгрузка Тогда
		...
		Если ИмяВременногоФайлаДанных = Неопределено Тогда
			...
			Сообщить(НСтр("ru = 'Не определен файл данных'"));
	...
	ФлагОтладкиОбработчиков = Объект.ФлагРежимОтладкиОбработчиков;
	ОбъектДляСервера = РеквизитФормыВЗначение("Объект");
	ОбъектДляСервера.ФлагРежимОтладки = ФлагРежимОтладкиСохраненный;
	ОбъектДляСервера.РежимОбмена = "Выгрузка";
	
	Если ФлагОтладкиОбработчиков Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			Сообщить(НСтр("ru = 'Файл внешней обработки отладчиков событий не существует на сервере'"));
			Возврат Неопределено;
		...
		Если Отказ Тогда
			...
			Сообщить(НСтр("ru = 'Не удалось выгрузить обработчики событий'"));
			Возврат "";
	...
	ОбъектДляСервера.ФлагРежимОтладкиОбработчиков = ФлагОтладкиОбработчиков;
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка Тогда
		...
		АдресФайлаДанных = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(Объект.ИмяФайлаОбмена), ЭтаФорма.УникальныйИдентификатор);
		УдалитьФайлы(Объект.ИмяФайлаОбмена);
...
Процедура ОткрытьФормуНастройкиОтладкиОбработчиков(ОбработчикиСобытийИзФайлаПравил)
	...
	ИмяФайлаВнешнейОбработкиОбработчиковСобытийСохраняемое = Объект.ИмяФайлаВнешнейОбработкиОбработчиковСобытий;
	РежимОтладкиАлгоритмовСохраняемый = Объект.РежимОтладкиАлгоритмов;
	ИмяФайлаПравилОбменаСохраняемый = Объект.ИмяФайлаПравилОбмена;
	ИмяФайлаОбменаСохраняемый = Объект.ИмяФайлаОбмена;
	
	ИмяЭтойФормы = ЭтаФорма.ИмяФормы;
	ИмяОбработки = Лев(ИмяЭтойФормы, ПоследнийРазделитель(ИмяЭтойФормы));
	ИмяВызываемойФормы = ИмяОбработки + "УправляемаяФормаНастройкиОтладкиОбработчиков";
	
	ФормаНастройкиОтладкиОбработчиков = ПолучитьФорму(ИмяВызываемойФормы,,ЭтаФорма);
	
	ФормаНастройкиОтладкиОбработчиков.ИмяФайлаВнешнейОбработкиОбработчиковСобытий = ИмяФайлаВнешнейОбработкиОбработчиковСобытийСохраняемое;
	ФормаНастройкиОтладкиОбработчиков.РежимОтладкиАлгоритмов = РежимОтладкиАлгоритмовСохраняемый;
	ФормаНастройкиОтладкиОбработчиков.ОбработчикиСобытийЧитаемИзФайлаПравилОбмена = ОбработчикиСобытийИзФайлаПравил;
	ФормаНастройкиОтладкиОбработчиков.Объект.ИмяФайлаПравилОбмена = ИмяФайлаПравилОбменаСохраняемый;
	ФормаНастройкиОтладкиОбработчиков.Объект.ИмяФайлаОбмена = ИмяФайлаОбменаСохраняемый;
	ФормаНастройкиОтладкиОбработчиков.ИмяОбработки = ИмяОбработки;
	
	Если ФормаНастройкиОтладкиОбработчиков.ОткрытьМодально() = Истина Тогда
		
		Объект.ИмяФайлаВнешнейОбработкиОбработчиковСобытий = ФормаНастройкиОтладкиОбработчиков.ИмяФайлаВнешнейОбработкиОбработчиковСобытий;
		Объект.РежимОтладкиАлгоритмов = ФормаНастройкиОтладкиОбработчиков.РежимОтладкиАлгоритмов; 
...
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимости()
	...
	#Если Не ВебКлиент  Тогда
		...
		Если Не ПустаяСтрока(Объект.ИмяФайлаПротоколаОбмена) Тогда
			ЗапуститьПриложение("explorer " + Объект.ИмяФайлаПротоколаОбмена);
		КонецЕсли;
		...
		Если Объект.НепосредственноеЧтениеВИБПриемнике Тогда
			...
			Если Не ПустаяСтрока(Объект.ИмяФайлаПротоколаОбменаЗагрузка) Тогда
				ЗапуститьПриложение("explorer " + Объект.ИмяПротоколаЗагрузки);
			КонецЕсли;
