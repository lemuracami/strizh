...
Процедура ВосстановитьНастройкиОбновления(ВыдаватьСообщения = Истина) Экспорт
...
Функция  ОпределитьВерсиюПлатформы()
...
Процедура ПроверитьНаличиеОбновлений() Экспорт 
	...
	ВосстановитьНастройкиОбновления(Ложь);
...
Функция ПолучитьДоступноеОбновление(Знач ВерсияКонфигурации) 
	...
	Отбор = Новый Структура("Конфигурация, Версия", 
		Метаданные.Имя, ВерсияКонфигурации);
...
Функция ЭтоСеансКонфигуратором(СеансИнформационнойБазы) Экспорт
	
	Возврат ВРег(СеансИнформационнойБазы.ИмяПриложения) = ВРег("Designer");
	
КонецФункции 

Функция ЭтоCOMСоединение(СеансИнформационнойБазы) Экспорт
...
Функция ПроверитьДоступКИБ(Знач ВыдаватьСообщения = Истина) Экспорт
	...
	Попытка
		...
		УточнитьПараметрыПодключенияККластеру(ВыдаватьСообщения);
...
Процедура УточнитьПараметрыПодключенияККластеру(ВыдаватьСообщения = Истина)

	НестандартныеПортыСервера = ПортКластераСерверов <> 0 ИЛИ ПортАгентаСервера <> 0;
	Если НестандартныеПортыСервера Или ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
		Возврат;
	КонецЕсли;
