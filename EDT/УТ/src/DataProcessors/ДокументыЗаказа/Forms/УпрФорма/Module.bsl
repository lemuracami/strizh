...
//	Объект.Контрагент = Справочники.Контрагенты.ПустаяСсылка();
	Объект.Контрагент = Справочники.ПолучателиЗаказов.ПустаяСсылка();
	//-Широков Задача 4495 11.03.2021
...
Функция ПолучитьДокументы(Номер)
	...
	Для Каждого Тек Из Рез Цикл
		...
		Если ТипЗнч(Тек.Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			...
			Объект.Контрагент = Тек.Параметр3; //Широков  в контрагенты присваивается число(!) новаТранспорт.НомерЛог
...
Функция НомерЗаказаПоВнешнемуНомеру(Номер)
	...
	СписокДоков.Параметры.УстановитьЗначениеПараметра("МасНомер", Формат(Объект.Номер, "ЧГ="));
...
Функция ПолучитьТабличныйДокументЗаказа(Таб, Доставка, Номер, МасДос)
	...
	Печ.ПечатьПоДоставке2(Доставка, Таб);
	//CeHbKA 03.06.2019
...
Процедура ГруппаВкладокПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Имя = "Задачи" Тогда
        Если  СписокЗадач.Параметры.Элементы[0].Значение<>Мас Тогда
            СписокЗадач.Параметры.УстановитьЗначениеПараметра("МасНомер", Мас);
            Элементы.СписокЗадач.Обновить();
		КонецЕсли;
	// Михушкин --->> 
	...
	// <<--- Михушкин 	
    Иначе
        Если  ЭтаФорма[ТекущаяСтраница.ПодчиненныеЭлементы[0].Имя].Параметры.Элементы[0].Значение <> Мас Тогда
            ЭтаФорма[ТекущаяСтраница.ПодчиненныеЭлементы[0].Имя].Параметры.УстановитьзНАчениеПараметра("МасНомер", Мас);
            Элементы[ТекущаяСтраница.ПодчиненныеЭлементы[0].Имя].Обновить();
        КонецЕсли;
	КонецЕсли;
...
Процедура НапечататьДоставку(Команда)
        ...
        Если Доставка <> Неопределено Тогда
			...
			Если ТипЗнч(Доставка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
				ОбработкаПечати.ПечатьПоДоставке2(Доставка, ТабличныйДокумент, Истина);
			ИначеЕсли ТипЗнч(Доставка) = Тип("ДокументСсылка.ЗаборТовара") Тогда	
				ОбработкаПечати.ПечатьЗабор(Доставка, ТабличныйДокумент);
			КонецЕсли; 			
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	НомерЗаказа = НайтиНомерЗаказаПоШК(НомерР);
	
	Если НомерЗаказа = Неопределено Тогда
		Сообщить("Заказ с ШК " + НомерР + " не найден");
	Иначе 
		Объект.Номер = НомерЗаказа;
		НомерПриИзменении(Неопределено);
	КонецЕсли;	
...
Функция НайтиНомерЗаказаПоШК(НомерР)
...
&НаСервереБезКонтекста
Процедура УстановкаПризнакаЗагрузкиНаСервере(НомерЗаказа)
	ТекстЗ = "declare @t [dbo].Torder ";
	ТекстЗ = ТекстЗ + "
	|insert into @t values (" + НомерЗаказа + ")";
...
&НаКлиенте
Процедура УстановкаПризнакаЗагрузки(Команда)
	УстановкаПризнакаЗагрузкиНаСервере(Формат(Объект.Номер, "ЧГ="));
КонецПроцедуры
