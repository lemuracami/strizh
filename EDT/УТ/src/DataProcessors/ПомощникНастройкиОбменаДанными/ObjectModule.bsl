...
Процедура ВыполнитьРегистрациюИзмененийПоУзлуИМетаданному(УзелДляРегистрацииИзменений, МетаданныеДляОбмена, 
	...
	Если (НЕ РегистрироватьВсеИзменения)
		...
		//регистрируем изменения в УТ
		ВозможныОрганиченияПоОрганизации = Организации.Количество() > 0;
	
		ВозможныОрганиченияПоДате = ЭтоДокумент И ЗначениеЗаполнено(ДатаНачалаВыгрузкиДокументов);	
	
		ВозможныОрганиченияПоСкладу = Склады.Количество() > 0 И ЭтоДокумент;
	
		ВозможныОрганиченияПоПодразделению = Подразделения.Количество() > 0 И ЭтоДокумент;
	
		ВозможныОграниченияПоОтражатьВБухгалтерскомУчете =  (МетаданныеДляОбмена.Реквизиты.Найти("ОтражатьВБухгалтерскомУчете") <> Неопределено) И ЭтоДокумент И ПередаватьТолькоДокументыОтраженныеВБухгалтерскомУчете;
	
		ВозможныОграниченияВыгрузкиСчета = (МетаданныеДляОбмена = Метаданные.Документы.СчетНаОплатуПокупателю ИЛИ МетаданныеДляОбмена = Метаданные.Документы.ЗаказПокупателя);

		
		Если НЕ ЭтоРегистр Тогда
			
			НаличиеОрганизации = (МетаданныеДляОбмена.Реквизиты.Найти("Организация") <> Неопределено);
			МассивРеквизитовПоСкладу = ОпределитьНаличиеСкладаВМетаданныхДляРегистрации(МетаданныеДляОбмена);
			НаличиеСклада      = МассивРеквизитовПоСкладу.Количество() > 0 И ВозможныОрганиченияПоСкладу;
			МассивРеквизитовПоПодразделению = ОпределитьНаличиеПодразделенияВМетаданныхДляРегистрации(МетаданныеДляОбмена);
			НаличиеПодразделения      = МассивРеквизитовПоПодразделению.Количество() > 0 И ВозможныОрганиченияПоПодразделению;
			
		Иначе
			
			НаличиеОрганизации   = (МетаданныеДляОбмена.Измерения.Найти("Организация") <> Неопределено);
			НаличиеСклада        = Ложь;
			НаличиеПодразделения = Ложь;
		...
		Если НаличиеОрганизации
			ИЛИ ВозможныОрганиченияПоДате
			ИЛИ НаличиеСклада
			ИЛИ НаличиеПодразделения
			ИЛИ ВозможныОграниченияВыгрузкиСчета 
			ИЛИ ВозможныОграниченияПоОтражатьВБухгалтерскомУчете Тогда
			...
			Если НаличиеОрганизации Тогда
				...
				ОграничениеВыборкиПоОрганизации = "РегистрацияИзменений.Организация В (&МассивОрганизаций)";
			...
			Если НаличиеСклада Тогда
				
				ОграничениеВыборкиПоСкладу = "(";
				
				Для каждого РеквизитСклад Из МассивРеквизитовПоСкладу Цикл
					
					ОграничениеВыборкиПоСкладу = ОграничениеВыборкиПоСкладу + "РегистрацияИзменений."+ РеквизитСклад + " В (&МассивСкладов) ИЛИ "
					
				КонецЦикла;
				//уберем последенее "ИЛИ"
				
				ОграничениеВыборкиПоСкладу = Лев(ОграничениеВыборкиПоСкладу,СтрДлина(ОграничениеВыборкиПоСкладу)-4);
				ОграничениеВыборкиПоСкладу = ОграничениеВыборкиПоСкладу + " )";
				
				Запрос.УстановитьПараметр("МассивСкладов", Склады.Выгрузить().ВыгрузитьКолонку("Склад"));
			...
			Если НаличиеПодразделения Тогда
				
				ОграничениеВыборкиПоПодразделению = "РегистрацияИзменений.Подразделение В (&МассивПодразделений)";
				
				Запрос.УстановитьПараметр("МассивПодразделений", Подразделения.Выгрузить().ВыгрузитьКолонку("Подразделение"));
				
			Иначе
				
				ОграничениеВыборкиПоПодразделению = "";
			...
			Если ВозможныОграниченияПоОтражатьВБухгалтерскомУчете Тогда
				
				ОграничениеВыборкиПоОтражатьВБухгалтерскомУчете = "РегистрацияИзменений.ОтражатьВБухгалтерскомУчете ";
				
			Иначе
				
				ОграничениеВыборкиПоОтражатьВБухгалтерскомУчете = "";
				
			КонецЕсли;
			
			Если ВозможныОграниченияВыгрузкиСчета Тогда
				...
				Если ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = Перечисления.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю.НеВыгружать Тогда
					...
					ОграничениеВыборкиПоВыгрузкеСчета = "ЛОЖЬ";	
				...
				ИначеЕсли ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = Перечисления.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю.ЗаказПокупателя
					И МетаданныеДляОбмена = Метаданные.Документы.СчетНаОплатуПокупателю  Тогда
					
					ОграничениеВыборкиПоВыгрузкеСчета = "ЛОЖЬ";	
					
				ИначеЕсли ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = Перечисления.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю.СчетНаОплатуПокупателю
					И МетаданныеДляОбмена = Метаданные.Документы.ЗаказПокупателя Тогда
					
					ОграничениеВыборкиПоВыгрузкеСчета = "ЛОЖЬ";		
					
				Иначе
					
					ОграничениеВыборкиПоВыгрузкеСчета = "ИСТИНА";					
			...
			Если ВозможныОграниченияВыгрузкиСчета Тогда
				...
				ОграничениеВыборки = ОграничениеВыборки + "
				|И " + ОграничениеВыборкиПоВыгрузкеСчета; 
			...
			Если ВозможныОграниченияПоОтражатьВБухгалтерскомУчете Тогда
				...
				ОграничениеВыборки = ОграничениеВыборки + "
				|И " + ОграничениеВыборкиПоОтражатьВБухгалтерскомУчете; 
			...
			Если НаличиеПодразделения Тогда
				...
				ОграничениеВыборки = ОграничениеВыборки + "
				|И " + ОграничениеВыборкиПоПодразделению; 
	...
	ИначеЕсли (НЕ РегистрироватьВсеИзменения)
		...
		Если ПодключениеКПриемнику <> Неопределено Тогда
			//регистрируем изменения в БП
			ВозможныОрганиченияПоОрганизации = УзелДляРегистрацииИзменений.Организации.Количество() > 0;
            ВозможныОграниченияВыгрузкиСчета = (МетаданныеДляОбмена = ПодключениеКПриемнику.Метаданные().Документы.СчетНаОплатуПокупателю);
			...
			ВозможныОрганиченияПоДате = ЭтоДокумент И ПодключениеКПриемнику.ЗначениеЗаполнено(УзелДляРегистрацииИзменений.ДатаНачалаВыгрузкиДокументов);
		
			НаличиеОрганизации = ВозможныОрганиченияПоОрганизации И НаличиеОрганизации; 

			Если НаличиеОрганизации
			ИЛИ  ВозможныОрганиченияПоДате
			ИЛИ  ВозможныОграниченияВыгрузкиСчета Тогда
			...
			Если НаличиеОрганизации Тогда
				...
				Запрос.УстановитьПараметр("МассивОрганизаций", УзелДляРегистрацииИзменений.Организации.Выгрузить().ВыгрузитьКолонку("Организация"));
			...
			Если ВозможныОрганиченияПоДате Тогда
				...
				Запрос.УстановитьПараметр("ДатаОграничений", УзелДляРегистрацииИзменений.ДатаНачалаВыгрузкиДокументов);
			...
			Если ВозможныОграниченияВыгрузкиСчета Тогда
				...
				Если УзелДляРегистрацииИзменений.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = ПодключениеКПриемнику.Перечисления.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю.НеВыгружать Тогда
			...
			Иначе
				...
				Если МетаданныеДляОбмена.РежимЗаписи = ПодключениеКПриемнику.Метаданные().СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
					
					МассивПолейВыбора.Добавить("Регистратор");
					
				Иначе
					
					Если МетаданныеДляОбмена.ПериодичностьРегистраСведений <> ПодключениеКПриемнику.Метаданные().СвойстваОбъектов.ПериодичностьРегистраСведений.НеПериодический Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					НаборЗаписейРегистра = ПодключениеКПриемнику.РегистрыСведений[МетаданныеДляОбмена.Имя].СоздатьНаборЗаписей();
					Для Каждого ЭлементПолей Из МассивПолейВыбора Цикл
						НаборЗаписейРегистра.Отбор[ЭлементПолей].Установить(Выборка[ЭлементПолей]);
					КонецЦикла;
...
Функция ОпределитьНаличиеСкладаВМетаданныхДляРегистрации(МетаданныеОбъекта)
	...
	Если МетаданныеОбъекта = Метаданные.Документы.АвансовыйОтчет
		...
		МассивРеквизитов.Добавить("СкладОрдер");
		
	ИначеЕсли МетаданныеОбъекта = Метаданные.Документы.ЗаказПокупателя Тогда
		
		МассивРеквизитов.Добавить("СкладГруппа");	
...
//возвращает массив имен реквизитов, которые могут участвовать в фильтрах
Функция ОпределитьНаличиеПодразделенияВМетаданныхДляРегистрации(МетаданныеОбъекта)
...
Функция ВыполнитьАнализСопоставленияОбъектов(ДляОтображения = Ложь) Экспорт
	...
	Если ТипНастройки = Перечисления.ТипыАвтоматическогоОбменаДанными.ОбменЧерезComСоединение Тогда
		...
		Запрос.УстановитьПараметр("ЭтотУзел",ПланыОбмена.ОбменУправлениеТорговлейБухгалтерия.ЭтотУзел().Код);
		МассивРезультатов = Запрос.ВыполнитьПакет();
				...
				Для каждого СтрокаУзлаСОМ Из СтрокаЗаголовкаСОМ.Строки цикл
					...
					Для каждого СтрокаОбъектаСОМ Из СтрокаУзлаСОМ.Строки цикл
						...
						Для каждого СтрокаДубляСОМ Из СтрокаОбъектаСОМ.Строки цикл
							...
							СтрокаДубля.УзелОбмена        = ПланыОбмена.ОбменУправлениеТорговлейБухгалтерия.НайтиПоКоду(СтрокаДубля.УзелОбмена.Код);
		...
		Попытка
			...
			Если ДействиеВыбранноеПользователем = 1 Тогда
				...
				Если ЗначениеЗаполнено(Узел) Тогда
					...
					"Для устранения проблемы в базе-источнике в плане обмена ""Обмен Управление торговлей-Бухгалтерия""" + Символы.ПС  + 
			...
			Если НужноСоздаватьНовыйУзел Тогда
				...
				Если ДействиеВыбранноеПользователем = 0 Тогда
					...
					Если НужныОграниченияПоОрганизации() Тогда
						...
						УзелПланаОбмена.Кассы.Загрузить(Кассы.Выгрузить());
				...
				УзелПланаОбмена.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю;
...
Функция СохранитьНастройкиПервогоЭтапа(НастройкаОбменаОбъект, ИнформацияОТипеОбмена, НужноРегистрироватьИзмененияДляОбмена = Истина) Экспорт
	...
	Попытка
		...
		//Если НужноВнестиИзмененияВИнформационнуюБазуПриемника Тогда 
		//	
		//	ПодключениеКПриемнику.ПолныеПрава.ОпределитьПараметрыСеансаДляОбменаДанными();
		//	
		//КонецЕсли;	
...
Процедура ОбновитьНастройкиВариантовПоискаОбъектов(ВариантОбмена  = "Новый") Экспорт
	Если ВариантОбмена = "Новый" Тогда
		ИмяПараметраВКонвертации = "ВариантыНастроекПоиска";	
	ИначеЕсли ВариантОбмена = "Старый" Тогда
		ИмяПараметраВКонвертации = "ВариантыНастроекПоискаДляПереходаСПредыдущегоОбмена";	
	КонецЕсли;
...
КонецПроцедуры	
//РАБОТА С ВАРИАНТАМИ СОПОСТАВЛЕНИЯ
...
Процедура ПрочитатьНастройкуОбменаДанными(НастройкаОбменаДанными) Экспорт
	...
	Подразделения.Загрузить(УзелИнформационнойБазы.Подразделения.Выгрузить());
	Склады.Загрузить(УзелИнформационнойБазы.Склады.Выгрузить());
	Кассы.Загрузить(УзелИнформационнойБазы.Кассы.Выгрузить());
	
	
	ПередаватьТолькоДокументыОтраженныеВБухгалтерскомУчете = УзелИнформационнойБазы.ПередаватьТолькоДокументыОтраженныеВБухгалтерскомУчете;
	ВыгружатьАналитикуПоСкладам = УзелИнформационнойБазы.ВыгружатьАналитикуПоСкладам;
	ВариантПереносаСебестоимостиСписанияВБухгалтериюПредприятия = УзелИнформационнойБазы.ВариантПереносаСебестоимостиСписанияВБухгалтериюПредприятия;
	ДатаНачалаВыгрузкиДокументов = УзелИнформационнойБазы.ДатаНачалаВыгрузкиДокументов;
	ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = УзелИнформационнойБазы.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю;
...
Функция СоздатьЭлементПланаОбменаВИнформационнойбазеПодключения(ПодключениеКПриемнику, ИмяПланаОбмена, КодЭлемента, Наименование) Экспорт
	...
	//УзелПланаОбмена.ПрефиксДляИБПриемника = ПрефиксДляИБПриемника;
	УзелПланаОбмена.ДатаНачалаВыгрузкиДокументов = ДатаНачалаВыгрузкиДокументов;
	
	// Значение перечисления ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю
	Если ЗначениеЗаполнено(ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю) Тогда
		ИндексЗначенияПеречисления = Перечисления.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю.Индекс(ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю);
		ОбъектМетаданныхЗначенияПеречисления = ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю.Метаданные().ЗначенияПеречисления.Получить(ИндексЗначенияПеречисления);
		ИмяЗначенияПеречисления = ОбъектМетаданныхЗначенияПеречисления.Имя;
		УзелПланаОбмена.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = ПодключениеКПриемнику.Перечисления.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю[ИмяЗначенияПеречисления];
	КонецЕсли;
	//
...
Функция СохранитьПараметрыНастройкиВФайл(НастройкаОбмена, СохранитьПравилаВФайл = Истина) Экспорт
	...
	ЗаписатьXML(ЗаписьXML, ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю, "ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю", НазначениеТипаXML.Явное);
