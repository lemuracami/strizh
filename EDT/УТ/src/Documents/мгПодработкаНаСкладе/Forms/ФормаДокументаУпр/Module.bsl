...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		Объект.Тариф = мгДоработки.ПолучитьТекущийТариф(Объект.Дата, ПредопределенноеЗначение("Справочник.мгНачисленияУдержания.ПодработкаНаСкладе"));
	КонецЕсли;
...
&НаКлиенте
Процедура ПодработкаНаСкладеФИОСотрудникаПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.ПодработкаНаСкладе.ТекущиеДанные;
