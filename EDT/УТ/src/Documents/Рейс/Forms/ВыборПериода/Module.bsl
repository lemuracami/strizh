...
Функция ПечатьСпискаРейсов()
    ...
    Запрос.Текст = 
        ...
        |	И ДокументРейс.ТерминалДоставки = &ТерминалДоставки @СтрДопУсловиеНеВыводитьЗаборы@
        |
        |УПОРЯДОЧИТЬ ПО
        |	НомерПалетты
        |АВТОУПОРЯДОЧИВАНИЕ";
	...
	Если НеВыводитьЗаборы Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "@СтрДопУсловиеНеВыводитьЗаборы@", " И ДокументРейс.РольРейса <> Значение(Справочник.РолиТранспорта.РольЗабораТовара)");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "@СтрДопУсловиеНеВыводитьЗаборы@", "");
	КонецеСли;
