...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Якурнов 26.07.2018 14:47:31
	ПолучитьНомераИВладельцевЗаказов();
...
// Якурнов 26.07.2018 14:46:17
&НаСервере
Процедура ПолучитьНомераИВладельцевЗаказов()
...
Процедура ЗаказыЗаказПриИзменении(Элемент)
	Если Элементы.Заказы.ТекущиеДанные <> Неопределено Тогда
		Если ЗначениеЗаполнено(Элементы.Заказы.ТекущиеДанные.Заказ) Тогда
			...
			Если ТипЗнч(Элементы.Заказы.ТекущиеДанные.Заказ) = Тип("ДокументСсылка.ЗаборТовара") Тогда
				Элементы.Заказы.ТекущиеДанные.ВидЗаказа = ПолучитьПеречисленияВидыЗаказовВероут("COLLECTION");
			ИначеЕсли ТипЗнч(Элементы.Заказы.ТекущиеДанные.Заказ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда	
				Элементы.Заказы.ТекущиеДанные.ВидЗаказа = ПолучитьПеречисленияВидыЗаказовВероут("DROP");
			КонецеСли;	
		КонецЕсли;
	КонецеСли;
КонецПроцедуры

// Якурнов 26.07.2018 14:25:29
&НаСервере
Функция ПолучитьПеречисленияВидыЗаказовВероут(ИмяПеречисления)
	Возврат Перечисления.ВидыЗаказовВероут[ИмяПеречисления];
КонецФункции
...
Функция ЗаказыПеретаскиваниеОбработатьПриемникИсточник(ПараметрыПеретаскивания)
	...
	Исключение
		...
		Мас = ОбработкаСобытийИУведомленийСервер.ВернутьМассивАдресатовПоСобытию(ПолучитьПеречислениеВидыСобытийУведомленияАбонентовРейсПерекидка_ОшибкаПерекидки());
...
// Якурнов 26.07.2018 14:21:00
&НаСервере
Функция ПолучитьПеречислениеВидыСобытийУведомленияАбонентовРейсПерекидка_ОшибкаПерекидки()
	Возврат Перечисления.ВидыСобытийУведомленияАбонентов.РейсПерекидка_ОшибкаПерекидки;
КонецФункции
