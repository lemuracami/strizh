...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Таб = ФормированияЧеков.Выгрузить();
	Таб.Свернуть("ТипОплаты", "СуммаПродажиТоваров, СуммаВозвратаТоваров, СуммаПоОтмененнымЧекам, СуммаПродажиУслуг");
	
	Движения.ЖурналККМ_КонтрольнаяЛента.Очистить();
	Движения.ЖурналККМ_КонтрольнаяЛента.Записывать = Истина;
	
	Для Каждого Тек Из Таб Цикл
		Нов = Движения.ЖурналККМ_КонтрольнаяЛента.Добавить();
		Нов.Период = Дата;
		Нов.КассаККМ = КассаККМ;
		Нов.Кассир = Ответственный;
		Нов.ТипОплаты = Тек.ТипОплаты;
		Нов.Сумма = Тек.СуммаПродажиТоваров + Тек.СуммаПродажиУслуг - Тек.СуммаВозвратаТоваров;
		Нов.СуммаВозврата = Тек.СуммаПоОтмененнымЧекам;
		Нов.НомерСмены = НомерСмены;
	КонецЦикла;	

	//Движения.СостояниеСменККМ.Очистить();
	//Движения.СостояниеСменККМ.Записывать = Истина;
	//Движение = Движения.СостояниеСменККМ.Добавить();
	//Движение.Период = Дата;
	//Движение.ККМ = КассаККМ;
