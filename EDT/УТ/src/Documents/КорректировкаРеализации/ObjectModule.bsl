...
Процедура ДвиженияПоРегиструСписанныеТоварыВозврат(
	...
	Для каждого Строка Из ТаблицаДвижений Цикл
		...
		Если СтруктураШапкиДокумента.ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам
			И НЕ ЗначениеЗаполнено(Строка.ЗаказСписания)
		Тогда
			Строка.ЗаказСписания = СтруктураШапкиДокумента.Сделка;//Для заполнения счета в регистре ПродажиСебестоимость
		КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации")
		И ДокументОснование.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение
		И ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение
	Тогда
		ДокументРеализации = Неопределено;
		ВызватьИсключение "Нельзя вводить документ на согласованную корректировку на основании другой согласованной корректировки!";
	КонецЕсли;
	#Если Клиент Тогда
