...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре) Экспорт
	...
	РезультатЗапросаПоТоварам = ОбщегоНазначения.СформироватьЗапросПоТабличнойЧастиПодарочныеПозиции(ЭтотОбъект, "Товары", СтруктураПолей,СтруктураСложныхПолей);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЭтоНовый() Тогда
		...
		//ДополнительныеСвойства.Вставить("ДатаДоИзменения", ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, "Дата"));
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Ссылка", Ссылка);
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Выборка.Следующий();
		ДополнительныеСвойства.Вставить("ДатаДоИзменения", Выборка.Дата);
		ДополнительныеСвойства.Вставить("СтатусИнтернетМагазинаДоИзменения", Выборка.СтатусИнтернетМагазина);
		//Асеев 23.03.2021 (Задача № 4503)<<<
	КонецЕсли;
	//Асеев 05.10.2020 (Задача № 4289)<<<
...
//Асеев 23.03.2021 (Задача № 4503)>>>
Функция ПолучитьДанныеДляФиксацииИзмененийПоЗаказу(парРейс)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Рейс", парРейс);
	Запрос.Текст =
	"ВЫБРАТЬ
...
Функция ПолучитьДанныеДляФиксацииЗаказОтменен()
...
//Асеев 30.11.2022 (Задача № 4289)>>>
Функция ПолучитьДанныеДляФиксацииДатаДоставки(ДатаПереноса)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(ВЗ_СостоянияЗаказов.ПричинаОтказа, НастройкиУчетаОбщиеСрезПоследних.ПричинаОтказаПереносаПриСменеДатыДоставкиЗаказаПартнеромВРейсе) КАК ПричинаОтказа,
	|	ЕСТЬNULL(ВЗ_СостоянияЗаказов.Инициатор, ЗНАЧЕНИЕ(Перечисление.ИнициаторыСостояния.ИнтернетМагазин)) КАК Инициатор
	|ИЗ
	|	(ВЫБРАТЬ ПЕРВЫЕ 1
	|		СостоянияЗаказов.ПричинаОтказа КАК ПричинаОтказа,
	|		СостоянияЗаказов.ПричинаОтказа.ИнициаторСостояния КАК Инициатор
	|	ИЗ
	...
	|		И СостоянияЗаказов.ДатаПереноса = &ДатаПереноса
	|	
	|	УПОРЯДОЧИТЬ ПО
	|		СостоянияЗаказов.Период УБЫВ) КАК ВЗ_СостоянияЗаказов
	|		ПОЛНОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаОбщие.СрезПоследних КАК НастройкиУчетаОбщиеСрезПоследних
	|		ПО (ИСТИНА)";
...
Процедура ПриЗаписи(Отказ)
    ...
    ////+++++Серегин М.В. 23.09.2015 11:54:13 
    //ТаблицаШтрихкодов = СформироватьШтрихКоды ();
    //Если ТаблицаШтрихкодов.Количество() > 0 Тогда
    //    //Движения по РС ШтрихкодыЗаказов
    //    ДвиженияШтрихкодыЗаказов(ТаблицаШтрихкодов);
    //КонецЕсли;
    ////-----Серегин М.В. 23.09.2015 11:54:15 
	
		
	//+++ БАО 22.06.2017 №792 - Костыль
	Попытка	
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ВТ_Исходные.НомерСтроки КАК НомерСтроки,
		|	РеализацияТоваровУслугТовары.НомерСтроки КАК НомерСтроки1
		|ИЗ
		|	ВТ_Исходные КАК ВТ_Исходные
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
		|		ПО ВТ_Исходные.НомерСтроки <> РеализацияТоваровУслугТовары.НомерСтроки
		|			И ВТ_Исходные.Номенклатура = РеализацияТоваровУслугТовары.Номенклатура
		|			И ВТ_Исходные.Цена = РеализацияТоваровУслугТовары.Цена
		|			И ВТ_Исходные.Ссылка = РеализацияТоваровУслугТовары.Ссылка";
		Запрос.УстановитьПараметр("Ссылка", Ссылка);
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			...
			Отпр = lem.ОтправитьСообщение(СписокПолучателей, "Задвоение в заказе", "Номер заказа - " + Формат(Номер,"ЧГ=0"), , "Логистическая компания ""Стриж""");
	...
	//Асеев 05.10.2020 (Задача № 4289)>>>
	ДатаДоИзменения = Неопределено;
	//Асеев 23.03.2021 (Задача № 4503)>>>
	СтатусИнтернетМагазинаДоИзменения = Неопределено;
	//Если ДополнительныеСвойства.Свойство("ДатаДоИзменения", ДатаДоИзменения) И НачалоДня(ДатаДоИзменения) <> НачалоДня(Дата) Тогда
	ДатаПереноса = НачалоДня(Дата);
	ФиксацияДатаДоставки = ДополнительныеСвойства.Свойство("ДатаДоИзменения", ДатаДоИзменения) И НачалоДня(ДатаДоИзменения) <> ДатаПереноса;
	ФиксацияЗаказОтменен = ДополнительныеСвойства.Свойство("СтатусИнтернетМагазинаДоИзменения", СтатусИнтернетМагазинаДоИзменения) И СтатусИнтернетМагазинаДоИзменения = 2 И СтатусИнтернетМагазина = 4;
	Если ФиксацияДатаДоставки Или ФиксацияЗаказОтменен Тогда
	//Асеев 23.03.2021 (Задача № 4503)<<<
		Сейчас = ТекущаяДата();
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Ссылка", Ссылка);
		...
		|			И (РейсЗаказы.Заказ = &Ссылка)
		|			И РейсЗаказы.Ссылка = Т_Рейс.Ссылка
		|			И (Т_Рейс.Проведен)
		|			И (НЕ РейсЗаказы.УдаленИзРейса)
		|ГДЕ
		|	новаОтчетВодителя.Рейс ЕСТЬ NULL";
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Если УчетКонтроляВремениСервер.РейсАктивен(Выборка.Рейс) Тогда
				//Асеев 23.03.2021 (Задача № 4503)>>>
				ДанныеДляФиксацииИзмененийПоЗаказу = ПолучитьДанныеДляФиксацииИзмененийПоЗаказу(Выборка.Рейс);
				Если ФиксацияЗаказОтменен Тогда
					ДанныеДляФиксацииЗаказОтменен = ПолучитьДанныеДляФиксацииЗаказОтменен();
					
					СтруктураПараметров = Новый Структура;
					СтруктураПараметров.Вставить("Период", Сейчас);
					СтруктураПараметров.Вставить("Заказ", Ссылка);
					СтруктураПараметров.Вставить("Реквизит", Перечисления.РеквизитыЗаказа.СостояниеЗаказаОтменен);
					СтруктураПараметров.Вставить("Рейс", Выборка.Рейс);
					СтруктураПараметров.Вставить("СтароеЗначение", "2");
					СтруктураПараметров.Вставить("НовоеЗначение", "4");
					...
					СтруктураПараметров.Вставить("Транспорт", ДанныеДляФиксацииИзмененийПоЗаказу.Транспорт);
					...
					ПараметрыДокумента.Вставить("Реализация", Ссылка);
					ПараметрыДокумента.Вставить("Дата", Сейчас);
					ПараметрыДокумента.Вставить("Водитель", ДанныеДляФиксацииИзмененийПоЗаказу.Водитель);
					ПараметрыДокумента.Вставить("Статус", Справочники.новаРезультатМестнойДоставки.НеВыполнена);
					ПараметрыДокумента.Вставить("ПричинаНеВыполненияДоставки", Справочники.ПричиныНеВыполненияДоставки.ОтказКлиентаБезЗаезда);
					ПараметрыДокумента.Вставить("ПричинаОтказаПереноса", ДанныеДляФиксацииЗаказОтменен.ПричинаОтказаПереносаПриОтменеЗаказаПартнеромВРейсе);
					ПараметрыДокумента.Вставить("Транспорт", ДанныеДляФиксацииИзмененийПоЗаказу.Транспорт);
					ПараметрыДокумента.Вставить("ДатаДоставки", Сейчас);
					ПараметрыДокумента.Вставить("ТипЗаказа", Перечисления.ТипыЗаказов.Доставка);
					ПараметрыДокумента.Вставить("ДатаВыгрузки", Сейчас);
					ПараметрыДокумента.Вставить("ИнициаторСостояния", Перечисления.ИнициаторыСостояния.ИнтернетМагазин);
					ПараметрыДокумента.Вставить("СтатусОтИМ", Истина);
					ПараметрыДокумента.Вставить("Рейс", Выборка.Рейс);
					
					РаботаСМобильнымПриложениемЭкипажейСервер.ЭмуляцияДанныеЗаказаВодителя(ПараметрыДокумента);
					//Асеев 23.03.2021 (Задача № 4503)<<<
					//Асеев 30.11.2022 (Задача № 4289)>>>
				ИначеЕсли ФиксацияДатаДоставки Тогда
					ДанныеДляФиксацииДатаДоставки = ПолучитьДанныеДляФиксацииДатаДоставки(ДатаПереноса);
					//Асеев 30.11.2022 (Задача № 4289)<<<
					//Асеев 23.03.2021 (Задача № 4503)<<<
					СтруктураПараметров = Новый Структура;
					СтруктураПараметров.Вставить("Период", Сейчас);
					СтруктураПараметров.Вставить("Заказ", Ссылка);
					СтруктураПараметров.Вставить("Реквизит", Перечисления.РеквизитыЗаказа.ДатаДоставки);
					СтруктураПараметров.Вставить("Рейс", Выборка.Рейс);
					СтруктураПараметров.Вставить("СтароеЗначение", Формат(ДатаДоИзменения, "ДФ=yyyyMMdd000000"));
					СтруктураПараметров.Вставить("НовоеЗначение", Формат(Дата, "ДФ=yyyyMMdd000000"));
					СтруктураПараметров.Вставить("СтатусТрансляцииИзменения", Перечисления.СтатусыТрансляцииИзмененийВЗаказах.ИзменениеЗафиксировано);
					//Асеев 23.03.2021 (Задача № 4503)>>>
					СтруктураПараметров.Вставить("Транспорт", ДанныеДляФиксацииИзмененийПоЗаказу.Транспорт);
					//Асеев 23.03.2021 (Задача № 4503)<<<
					...
					//Асеев 19.04.2021 (Задача № 4289)>>>
					ПараметрыДокумента = Новый Структура;
					ПараметрыДокумента.Вставить("Реализация", Ссылка);
					ПараметрыДокумента.Вставить("Дата", Сейчас);
					ПараметрыДокумента.Вставить("Водитель", ДанныеДляФиксацииИзмененийПоЗаказу.Водитель);
					ПараметрыДокумента.Вставить("Статус", Справочники.новаРезультатМестнойДоставки.НеВыполнена);
					ПараметрыДокумента.Вставить("ПричинаНеВыполненияДоставки", Справочники.ПричиныНеВыполненияДоставки.ПереносДоставки);
					...
					//Асеев 30.11.2022 (Задача № 4289)>>>
					ПараметрыДокумента.Вставить("ПричинаОтказаПереноса", ДанныеДляФиксацииДатаДоставки.ПричинаОтказа);
					ПараметрыДокумента.Вставить("ИнициаторСостояния", ДанныеДляФиксацииДатаДоставки.Инициатор);
					//Асеев 30.11.2022 (Задача № 4289)<<<
					
					ПараметрыДокумента.Вставить("СтатусОтИМ", Истина);
					ПараметрыДокумента.Вставить("ДатаПереноса", Дата);
					ПараметрыДокумента.Вставить("Рейс", Выборка.Рейс);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//ЭлементБлокировки.Область = "РегистрНакопления.ВзаиморасчетыСКонтрагентами";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.УстановитьЗначение("Контрагент", Контрагент);
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.УстановитьЗначение("Контрагент", Контрагент);
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	...
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.НДСНачисленный";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.УстановитьЗначение("Покупатель", Контрагент);
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.НДСПартииТоваров";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.НДСПредъявленный";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.УстановитьЗначение("Поставщик", Контрагент);;
	//
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.НДСПредъявленныйРеализация0";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.УстановитьЗначение("ДокументОтгрузки", Ссылка);
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.НДСРасчетыСПокупателями";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.УстановитьЗначение("Покупатель", Контрагент);
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ПартииТоваровНаСкладах";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ПартииТоваровПереданные";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ПредоставленныеСкидки";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.УстановитьЗначение("ПолучательСкидки", Контрагент);
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	...
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ПродажиСебестоимость";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	...
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.РеализованныеТовары";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ТоварыВРезервеНаСкладах";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ТоварыВРознице";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ТоварыКПередачеОрганизаций";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ТоварыКПередачеСоСкладов";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	//
	...
	//
	//ЭлементБлокировки = Блокировка.Добавить();
	//ЭлементБлокировки.Область = "РегистрНакопления.ТоварыПереданные";
	//ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	//ЭлементБлокировки.ИсточникДанных = Товары;
	//ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	
	////ЭлементБлокировки = Блокировка.Добавить();
	////ЭлементБлокировки.Область = "РегистрНакопления.ПодарочныеПозиции";
	////ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	////ЭлементБлокировки.ИсточникДанных = ПодарочныеПозиции;
	////ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
	////
	//////+++ БАО 02.08.2017 №1334 
	////ЭлементБлокировки = Блокировка.Добавить();
	////ЭлементБлокировки.Область = "РегистрНакопления.ПодарочныеПозиции";
	////ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	////ЭлементБлокировки.УстановитьЗначение("ИнтернетМагазин",ВладелецТовара);
	//////--- БАО 02.08.2017 №1334 
	////
	////
	...
	//КонецЕсли;
	//Если СтатусИнтернетМагазина = 3 Тогда
	...
	//Если Не Отказ Тогда
		...
		//Движения.ПодарочныеПозиции.Записывать = Истина;
		//
		//Для Каждого Тек Из ПодарочныеПозиции Цикл
		//	Нов = Движения.ПодарочныеПозиции.Добавить();
		//	Нов.Период = Дата;
		//	Нов.ИнтернетМагазин = ВладелецТовара;
		...
		//
		//Най = БизнесПроцессы.новаМестнаяДоставка.НайтиПоНомеру(Номер);
		//СумаКас = 0;
		//Если ЗначениеЗаполнено(Най) Тогда
		//	СумКас = Най.СуммаКассовогоОбслуживания;
		//КонецеСли;	
		//
		...
		//Нов.СтоимостьДоставки = Услуги.Итог("Сумма");
		//Нов.УслугиСД = УслугиСД;
		//Нов.КассовоеОбслуживание = СумКас;
		//Нов.ВидДвижения = ВидДвиженияНакопления.Приход;
		//Нов.Контрагент = ВладелецТовара;
	//КонецЕсли;
	//КонецеСли;
...
//CeHbKA #3587 
Процедура СформироватьДвиженияПоШтрихкодам_Новая(МассивСтруктурШК) Экспорт
    //+++++Серегин М.В. 23.09.2015 11:54:13 
    ТаблицаШтрихкодов = СформироватьШтрихКоды ();
	
	ТаблицаШтрихкодов.Колонки.Добавить("ШКПереданПартнером");
	
    //+++++Серегин М.В. 18.02.2016 16:37:05 
	//ТекстЗапросаSQL = "EXEC p1c_getBarcodes "+СокрЛП(Номер);
	//ТаблицаШтрихкодовАдминка = smv.СформироватьТаблицуЗначенийИзДанныхSQL(ТекстЗапросаSQL);
    Для каждого СтруктураШК Из МассивСтруктурШК Цикл
        СтрокаТЗ = ТаблицаШтрихкодов.Добавить();
        СтрокаТЗ.Штрихкод = СтруктураШК.barcode;
		СтрокаТЗ.НомерМеста = СтруктураШК.piece;
		СтрокаТЗ.ШКПереданПартнером = Истина;
    КонецЦикла;
    //-----Серегин М.В. 18.02.2016 16:37:07 
	...
	//lem, все таки делаем по отдельным контрагентам
	//// МАС - 06.09.2017 - №костыль для ООО Арвин (нового контрагента вместо АвтоОлл)  --->> 
	
	
	Если ВладелецТовара.Код = "Shop_713 " Или ВладелецТовара.Код = "Shop_729 " Или ВладелецТовара.Код = "Shop_730 " Тогда	
		Если ЗначениеЗаполнено(НомерВнешнегоЗаказа) Тогда
			
			Наб = РегистрыСведений.ШтрихкодыЗаказов.СоздатьНаборЗаписей();
			Наб.Отбор.Заказ.Установить(Ссылка);
			Наб.Отбор.Штрихкод.Установить(СокрЛП(НомерВнешнегоЗаказа));
			Наб.Отбор.ДокументТСД.Установить(Документы.ЗагрузкаСТСД.ПустаяСсылка());
			Наб.Прочитать();
			
			Если НЕ Наб.Количество() Тогда			
				Нов = Наб.Добавить();
				Нов.Заказ = Ссылка;
				Нов.Штрихкод = СокрЛП(НомерВнешнегоЗаказа);
				Наб.Записать();				
			КонецЕсли;	
...
Процедура СформироватьДвиженияПоШтрихкодам() Экспорт
    //+++++Серегин М.В. 23.09.2015 11:54:13 
    ТаблицаШтрихкодов = СформироватьШтрихКоды ();
    ...
    ТекстЗапросаSQL = "EXEC p1c_getBarcodes "+СокрЛП(Номер);
    ТаблицаШтрихкодовАдминка = smv.СформироватьТаблицуЗначенийИзДанныхSQL(ТекстЗапросаSQL);
    Для каждого Стр Из ТаблицаШтрихкодовАдминка Цикл
        Строка = ТаблицаШтрихкодов.Добавить();
        Строка.Штрихкод = Стр.barcode;
		Строка.НомерМеста = Стр.piece;
		Строка.ШКПереданПартнером = Истина;
    КонецЦикла;
    //-----Серегин М.В. 18.02.2016 16:37:07 
...
Функция СформироватьШтрихКоды() Экспорт
    ВыборкаАлгоритмов = ПолучитьВыборкуАлгоритмов(НЕ ЗначениеЗаполнено(НомерВнешнегоЗаказа));
    ТаблицаШтрихкодов = Новый ТаблицаЗначений;
    ТаблицаШтрихкодов.Колонки.Добавить("Штрихкод");
	ТаблицаШтрихкодов.Колонки.Добавить("НомерМеста");
    
    Пока ВыборкаАлгоритмов.Следующий() Цикл
        Строка = ТаблицаШтрихкодов.Добавить();
        Штрихкод = "";
        Выполнить(ВыборкаАлгоритмов.Шаблон);
        Штрих  = СокрЛП(ВыборкаАлгоритмов.НачальныйСимвол)+Штрихкод; 
        Строка.Штрихкод = Штрих;
		Строка.НомерМеста = 1;
    КонецЦикла;
    ТаблицаШтрихкодов.Свернуть("Штрихкод,НомерМеста");
	Возврат ТаблицаШтрихкодов;

КонецФункции // Сформиров()

Функция ПолучитьВыборкуАлгоритмов(ШаблонПоУмолчанию) Экспорт

    Запрос = Новый Запрос;
    Запрос.Текст = 
        "ВЫБРАТЬ
        |   ШаблоныШтрихкодовКонтрагентов.Шаблон.НачальныйСимвол КАК НачальныйСимвол,
        |   ШаблоныШтрихкодовКонтрагентов.Шаблон.Шаблон КАК Шаблон
        |ПОМЕСТИТЬ ВТ_Объединение
        |ИЗ
        |   РегистрСведений.ШаблоныШтрихкодовКонтрагентов КАК ШаблоныШтрихкодовКонтрагентов
        |ГДЕ
        |   ШаблоныШтрихкодовКонтрагентов.Контрагент = &ВладелецТовара
        |   И НЕ &ШаблонПоУмолчанию
        |
        |ОБЪЕДИНИТЬ ВСЕ
        |
        |ВЫБРАТЬ
        ...
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ
        |   ВТ_Объединение.НачальныйСимвол,
    ...
    Запрос.УстановитьПараметр("ШаблонПоУмолчанию", ШаблонПоУмолчанию);
    Возврат Запрос.Выполнить().Выбрать();
    
КонецФункции // ПолучитьТаблицуАлгоритмов()

Процедура ОбработатьПачкуШтрихкодовЗаказа(ТаблицаШтрихкодов) // Задача № 3189
	
	Если СокрЛП(ВладелецТовара.Код) = "Shop_428" Тогда // Отключение обработки штрихкодов для ИМ Детский Мир (задача 3189, комментарий ЕМ 16.10.2019 9:15)
		Возврат;
	КонецЕсли;	
	
	СписокШтрихкодов = ТаблицаШтрихкодов.ВыгрузитьКолонку("Штрихкод");
	...
	Запрос.Текст = 
		...
		|	МестаПоЗаказам.Заказ КАК ЗаказСправ,
		|	ШтрихкодыЗаказов.Заказ КАК ЗаказРС,
		...
		|		ПО МестаПоЗаказам.Заказ = ШтрихкодыЗаказов.Заказ
		|			И МестаПоЗаказам.Штрихкод = ШтрихкодыЗаказов.Штрихкод
		|ГДЕ
		|	МестаПоЗаказам.Заказ = &Заказ
		|	И НЕ МестаПоЗаказам.НеАктуальное
		|	И НЕ МестаПоЗаказам.Штрихкод В (&СписокШтрихкодов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_Штрихкоды.Ссылка КАК Ссылка,
		|	ВТ_Штрихкоды.Штрихкод КАК Штрихкод,
		|	ВТ_Штрихкоды.ЗаказСправ КАК ЗаказСправ,
		|	ВТ_Штрихкоды.ЗаказРС КАК ЗаказРС,
		|	ВТ_Штрихкоды.ШКПереданПартнером КАК ШКПереданПартнером
		|ИЗ
		|	ВТ_Штрихкоды КАК ВТ_Штрихкоды
		|ГДЕ
		|	НЕ ВТ_Штрихкоды.ЗаказРС ЕСТЬ NULL
		|	И ВТ_Штрихкоды.ШКПереданПартнером";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		МестаПоЗаказамОбъект = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		МестаПоЗаказамОбъект.НеАктуальное = Истина;
		МестаПоЗаказамОбъект.Штрихкод = "-" + МестаПоЗаказамОбъект.Штрихкод;
		МестаПоЗаказамОбъект.Записать();
		
	КонецЦикла;	
			
	НаборЗаписей = РегистрыСведений.ШтрихкодыЗаказов.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Заказ.Установить(ЭтотОбъект.Ссылка);
	//Геннадий #3189 12.08.2020 ->
	//дополнение к задаче №3
	//НаборЗаписей.Отбор.ШКПереданПартнером.Установить(Истина);
	//Геннадий #3189 12.08.2020 <- 
	//Асеев 17.03.2021 (Задача № 3189)>>>
	//НаборЗаписей.Прочитать();
	//НаборЗаписей.Очистить();
	НаборЗаписей.Отбор.ДокументТСД.Установить(Документы.ЗагрузкаСТСД.ПустаяСсылка());
	//Асеев 17.03.2021 (Задача № 3189)<<<
	НаборЗаписей.Записать();
		
КонецПроцедуры	

Процедура ДвиженияШтрихкодыЗаказов(ТаблицаШтрихкодов) Экспорт
	
	// МАС - 03.08.2017 - № --->> 
	//НаборЗаписей = РегистрыСведений.ШтрихкодыЗаказов.СоздатьНаборЗаписей();
	//НаборЗаписей.Отбор.Заказ.Установить(ЭтотОбъект.Ссылка);
	//
	//Для каждого Стр Из ТаблицаШтрихкодов Цикл
	//    Запись = НаборЗаписей.Добавить();
	//    Запись.Заказ = ЭтотОбъект.Ссылка;
	//    Запись.Штрихкод = Стр.Штрихкод;
	//	Запись.НомерМеста = Стр.НомерМеста;
	//КонецЦикла;
	//НаборЗаписей.Записать();	
	
	ОбработатьПачкуШтрихкодовЗаказа(ТаблицаШтрихкодов); // Задача № 3189
	...
	Для каждого Стр Из ТаблицаШтрихкодов Цикл
        ...
        Запись.Штрихкод = Стр.Штрихкод;
		Запись.НомерМеста = Стр.НомерМеста;
		Запись.ШКПереданПартнером = Стр.ШКПереданПартнером;
		НаборЗаписей.Записать();
