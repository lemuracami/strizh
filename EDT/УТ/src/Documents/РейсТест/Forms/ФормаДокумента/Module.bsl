...
Функция ЗаказыПеретаскиваниеОбработатьПриемникИсточник(ПараметрыПеретаскивания)
	...
	Исключение
		...
		Мас = ОбработкаСобытийИУведомленийСервер.ВернутьМассивАдресатовПоСобытию(Перечисления.ВидыСобытийУведомленияАбонентов.РейсПерекидка_ОшибкаПерекидки);
...
Процедура ЗаказыЗаказОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		РезультатПроверки = ПроверкаВыбранногоЗаказа(ВыбранноеЗначение, Объект.Дата, Объект.Ссылка);
