...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Проведен Тогда
		Элементы.ГруппаРегион.ТолькоПросмотр = Истина;
		Элементы.ФормаКомандаЗаполнитьИРассчитать.Доступность = Ложь;
		Элементы.ТарифРасчетаЗП.ТолькоПросмотр = Истина;
	Иначе
		Элементы.ТарифРасчетаЗП.ТолькоПросмотр = Не РольДоступна("Админы");
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ЗаполнитьИРассчитатьНаСервере()
	
	Если ЗначениеЗаполнено(Объект.Регион) И ЗначениеЗаполнено(Объект.ПериодРасчета) Тогда
		ДокОбъект = РеквизитФормыВЗначение("Объект");
		Результат = ДокОбъект.ЗаполнитьИРассчитать(Объект.Регион, Объект.ПериодРасчета);
...
Процедура ЗаполнитьИРассчитать(Команда)
	...
	Результат = ЗаполнитьИРассчитатьНаСервере();
