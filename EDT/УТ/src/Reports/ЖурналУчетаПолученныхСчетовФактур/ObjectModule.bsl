...
Процедура ВыводСтроки(СчетФактура, Секция, МассивПрефиксовДляРИБиОрганизации)
	...
	Если СчетФактура.ОпределитьПараметрыСчетаФактуры Тогда
		...
		Пока Документ.Следующий() Цикл
			...
			Иначе
				Секция.Параметры.ДатаНомер = Формат(Документ.ДатаВходящегоДокумента, "ДФ=dd.MM.yyyy") + ", № " + Документ.НомерВходящегоДокумента;
			КонецЕсли; 
		КонецЦикла;
	...
	Иначе
		...
		Пока Документ.Следующий() Цикл
			Если УстановитьПараметры Тогда
				УстановитьПараметры = Ложь;
				Секция.Параметры.Заполнить(Документ);
				Секция.Параметры.КонтрагентНаименование = СокрЛП(Документ.КонтрагентНаименование);
				Секция.Параметры.Сумма = ?(СчетФактура.СуммаДокумента = 0, "", Формат(СчетФактура.СуммаДокумента, "ЧЦ=19; ЧДЦ=2") + " " + Строка(Документ.ВалютаДокумента));
			КонецЕсли;					
			Если ПустаяСтрока(ПредставлениеОснования) Тогда
				Секция.Параметры.ДокументОснование = Документ.ДокументОснование;
			Иначе
