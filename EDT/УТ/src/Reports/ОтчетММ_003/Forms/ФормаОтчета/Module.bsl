...
Процедура ОтправитьПоПочте(Команда)
	...
	КомпоновщикНастроек.ЗагрузитьНастройки(СтруктураВариантов.ДляПолученияКонтрагентов);
	...
	МасК = Таб.Строки.ВыгрузитьКолонку("Контрагент");
	Зап = Новый Запрос;
	Зап.Текст = "ВЫБРАТЬ
...
/////////////////	
	...
	Для Каждого Тек Из Таб.Строки Цикл
		...
		КомпоновщикНастроек.Настройки.параметрыданных.Элементы[0].Значение = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение;
		КомпоновщикНастроек.Настройки.отбор.элементы[0].ПравоеЗначение = Тек.Контрагент.Ссылка;
		КомпоновщикНастроек.Настройки.отбор.элементы[0].Использование = Истина;
		...
		Стр = "Отчет ММ_003 "+Тек.Контрагент + ПредставлениеПериода(Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.ДатаНачала, Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.Датаокончания, "ДЛФ=DD");
		//Т.ПолучитьТекст();
		МасП = Новый Массив;
		
		//МасП.Добавить("pavel.nechaev@strizh-logistic.ru");
        МасП.Добавить("logist@strizh-logistic.ru");
        //+++++Серегин М.В. 19.01.2016 11:54:42 
       // МасП.Добавить("yuriy.gnedov@strizh-logistic.ru");
        //-----Серегин М.В. 19.01.2016 11:54:43 
        //МасП.Добавить("michail.seregin@strizh-logistic.ru");
		...
		Отпр = lem.ОтправитьСообщение(МасП, "Отчет ММ_003 "+Тек.Контрагент + ПредставлениеПериода(Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.ДатаНачала, Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.Датаокончания, "ДЛФ=DD"), Стр, ПутьКФайлу, "Логистическая компания ""Стриж""");
		Сообщить("Вывод данных по " + СокрЛП(Тек.Контрагент.Наименование));
	КонецЦикла;
...
Процедура ОтправитьПоПочтеРуководству()
	...
	Т = Новый ТекстовыйДокумент();
	Т.Прочитать(ПутьКФайлу);
	Стр = "Отчет ММ_003 " + ПредставлениеПериода(Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.ДатаНачала, Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.Датаокончания, "ДЛФ=DD");
	МасП = Новый Массив;
    ...
    МасП.Добавить("logist@strizh-logistic.ru");
    //+++++Серегин М.В. 19.01.2016 11:55:01 
	//МасП.Добавить("yuriy.gnedov@strizh-logistic.ru");
    //-----Серегин М.В. 19.01.2016 11:55:02 
    ...
    //МасП.Добавить("79251486565@yandex.ru");
	//МасП.Добавить("evgeniy.marochkin@strizh-logistic.ru");
	
	//Отпр = lem.ОтправитьСообщение(МасП, "Отчет по отказам/переносам за " + ПредставлениеПериода(Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.ДатаНачала, Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.Датаокончания, "ДЛФ=DD"), Стр, , "Логистическая компания ""Стриж""",, ТипТекстаПочтовогоСообщения.HTML) ;
	
	Отпр = lem.ОтправитьСообщение(МасП, "Отчет ММ_003 " + ПредставлениеПериода(Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.ДатаНачала, Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.Датаокончания, "ДЛФ=DD"), Стр, ПутьКФайлу, "Логистическая компания ""Стриж""") ;
КонецПроцедуры
