...
Функция СформироватьОтправитьПоПочтеСОтборомПоКонтрагентам(НачПериода, КонПериода, КонтрагеныИсключения, КонтрагентыЦелевые) Экспорт
	
	СхемаКомпоновкиДанных = ПолучитьМакет("СхемаРегламент2");
	...
	//Добавляем отбор если есть КонтрагентыЦелевые в параметрах
	Если КонтрагентыЦелевые <> Неопределено Тогда
		...
		НовыйЭлементОтбора.ПравоеЗначение = КонтрагентыЦелевые;
	Иначе
		//Добавляем отбор если есть КонтрагеныИсключения в параметрах
		Если КонтрагеныИсключения <> Неопределено Тогда
			НовыйЭлементОтбора = Настройки.Отбор.Элементы.
