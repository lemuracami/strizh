...
Процедура ВывестиВОтчетДанныеПоБазеРаспределения(ДокументРезультат, Макет)
	...
	УчетНДС.РассчитатьВыручкуДляНДС(Организация, ДатаНач, ДатаКон, ВыручкаЕНВД, ВыручкаБезНДС, ВыручкаНДС0, ВыручкаНДС);
...
Процедура ВывестиСтрокуТабличнойЧасти(ДокументРезультат, ОбластьСтрока, Выборка)
	...
	Иначе
		Документ = УчетНДС.НайтиПодчиненныйДокумент(Выборка.СчетФактура, "СчетФактураПолученный");
