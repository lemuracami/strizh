...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если (Отбор.План.Значение = Ложь) И (Отбор.План.Использование)
		...
		ЗапросРасчетаОсновногоЗначения.УстановитьПараметр("ИсточникДанных", ЭтотОбъект.Выгрузить());
		...
		|ИЗ
		|	&ИсточникДанных КАК ЗаписьНабора";
		...
		Если Не ЗапросРасчетаОсновногоЗначения.Выполнить().Пустой() Тогда // применяем индивидуальный график
		...
		Иначе // применяем общие графики
			...
			|	Даты.Дата КАК Дата
			|ПОМЕСТИТЬ ВТДатыСрезаПоследних
			|ИЗ
			|	ВТДаты КАК Даты
			...
			|	Даты.Дата КАК Дата,
			|	ВЫБОР
			|		КОГДА Даты.ДатаОпределенияГрафика ЕСТЬ NULL 
			|			ТОГДА РаботникиОрганизаций.ГрафикРаботы
			|		КОГДА РаботникиОрганизаций.ПериодЗавершения <= Даты.Дата
			...
			|ИЗ
			|	ВТДатыСрезаПоследних КАК Даты
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДатыСрезаПервых КАК ОпределениеГрафикаСрезПервых
			|		ПО Даты.Дата = ОпределениеГрафикаСрезПервых.Дата
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
			|		ПО (РаботникиОрганизаций.Сотрудник = &Сотрудник)
			|			И (ЕСТЬNULL(Даты.ДатаОпределенияГрафика, ОпределениеГрафикаСрезПервых.ДатаОпределенияГрафика) = РаботникиОрганизаций.Период)
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Дата,
			|	ГрафикРаботы
			...
			|		ПО ЗаписьНабора.Дата = ДатыГрафики.Дата
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботыПоВидамВремени КАК РасчетОсновногоЗначения
